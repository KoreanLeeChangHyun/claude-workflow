...
- **alias**: `flow-claude`
- **스크립트**: `.claude.workflow/scripts/banner/flow_claude_banner.sh`
...
- **alias**: `flow-step`
- **스크립트**: `.claude.workflow/scripts/banner/flow_step_banner.sh`
...
- **alias**: `flow-phase`
- **스크립트**: `.claude.workflow/scripts/banner/flow_phase_banner.sh`
...
- **alias**: `flow-init`
- **스크립트**: `.claude.workflow/scripts/flow/initialization.py`
...
| 변수 | 설명 |
|------|------|
| `TICKET_NUMBER` | 티켓 번호 (T-NNN 또는 NNN 형식). 미지정 시 .claude.workflow/kanban/active/에서 자동 선택 |
...
- **alias**: `flow-reload`
- **스크립트**: `.claude.workflow/scripts/flow/reload_prompt.py`
...
| 인자 | 필수 | 타입 | 설명 |
|------|------|------|------|
| `workDir` | 필수 | string | 작업 디렉터리 상대 경로 (예: `.claude.workflow/workflow/20260325-004729/작업명/implement`) |

#### 환경변수

| 변수 | 설명 |
|------|------|
| `TICKET_NUMBER` | 티켓 번호 (T-NNN 또는 NNN). 미지정 시 .context.json 또는 .claude.workflow/kanban/active/ 스캔으로 자동 탐색 |
...
```bash
flow-reload .claude.workflow/workflow/20260325-004729/로그인-버그-수정/implement
```
...
- **alias**: `flow-update`
- **스크립트**: `.claude.workflow/scripts/flow/update_state.py`
...
| `env` | `flow-update env <registryKey> set\|unset <KEY> [VALUE]` | .claude.workflow/.env 환경변수 관리 |
...
- **alias**: `flow-skillmap`
- **스크립트**: `.claude.workflow/scripts/flow/skill_mapper.py`
...
- **alias**: `flow-skill`
- **스크립트**: `.claude.workflow/scripts/flow/skill_state_manager.py`
...
- **alias**: `flow-validate`
- **스크립트**: `.claude.workflow/scripts/flow/plan_validator.py`
...
```bash
flow-validate .claude.workflow/workflow/20260325-004729/로그인-버그-수정/implement/plan.md
```
...
- **스크립트**: `.claude.workflow/scripts/flow/prompt_validator.py`
...
| 인자 | 필수 | 타입 | 설명 |
|------|------|------|------|
| `prompt_file_path` | 필수 | string | 티켓 XML 파일 경로 (.claude.workflow/kanban/active/T-NNN.xml) |
...
```bash
flow-validate-p .claude.workflow/kanban/active/T-169.xml
```
...
- **스크립트**: `.claude.workflow/scripts/flow/skill_recommender.py`
...
- **alias**: `flow-gc`
- **스크립트**: `.claude.workflow/scripts/flow/garbage_collect.py`
...
- `.claude.workflow/workflow/` 하위에서 TTL 만료 + 미완료(DONE/FAILED/STALE 미해당) status.json을 STALE로 전환
...
- **alias**: `flow-kanban`
- **스크립트**: `.claude.workflow/scripts/flow/kanban.py`
- **설명**: 칸반 보드 상태 관리 CLI. XML 티켓 파일(.claude.workflow/kanban/active/T-NNN.xml)을 SSoT로 사용

#### 서브커맨드

| 서브커맨드 | 사용법 | 설명 |
|-----------|-------|------|
| `create` | `flow-kanban create "<title>" [--command <cmd>]` | 새 티켓 생성 (.claude.workflow/kanban/active/T-NNN.xml) |
