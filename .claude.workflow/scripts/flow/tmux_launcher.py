...
def cmd_launch(ticket_id: str, command: str) -> int:
    ...
    cwd = os.getcwd()
    if os.path.join(".claude.workflow", "worktrees") + os.sep in cwd or cwd.endswith(os.path.join(".claude.workflow", "worktrees")):
        # cwd에서 T-NNN 추출: worktrees/feat-T-NNN-* 패턴
        import re as _re
        _wt_match = _re.search(r"\.claude\.workflow[/\\]worktrees[/\\][^/\\]*feat-(T-\d+)", cwd)
