...
.claude.workflow/.env 파일의 환경 변수를 set/unset하는 책임을 담당한다.
HOOK_*, GUARD_* 접두사 및 HOOKS_EDIT_ALLOWED 키만 허용하는
화이트리스트 기반 환경변수 관리를 수행한다.

책임 범위:
    - .claude.workflow/.env 환경변수 설정 (set)
    - .claude.workflow/.env 환경변수 해제 (unset)
    - KEY 화이트리스트 검증
    - 원자적 파일 쓰기
"""
...
def env_manage(action: str, key: str, value: str = "") -> str:
    ...
    env_file: str = os.path.join(PROJECT_ROOT, ".claude.workflow", ".env")
    if not os.path.isfile(env_file):
        print(f"[WARN] env: .claude.workflow/.env not found: {env_file}", file=sys.stderr)
