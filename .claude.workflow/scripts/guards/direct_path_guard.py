...
PreToolUse(Bash) 이벤트에서 python3 .claude.workflow/scripts/ 패턴의 직접 경로 호출을
...
def _extract_script_name(command: str) -> str | None:
    """명령어에서 .claude.workflow/scripts/ 하위 스크립트 파일명을 추출한다.
...
def main() -> None:
    ...
    stdin에서 JSON을 읽어 Bash 도구의 python3 .claude.workflow/scripts/ 직접 호출을 감지하고,
    ...
    if script_name and script_name in ALIAS_MAP:
        ...
        # 스크립트명 추출 실패 시 일반 차단
        _deny(DIRECT_PATH_CALL_DENIED.format(
            script_name=".claude.workflow/scripts/...",
            alias_name="flow-*",
        ))
