...
PreToolUse(Write|Edit|Bash) 이벤트에서 .claude.workflow/hooks/ 경로 파일 수정을 차단.
...
우회: 환경변수 HOOKS_EDIT_ALLOWED=1 설정 시 차단 해제
      (오케스트레이터가 `.claude.workflow/scripts/flow/update_state.py env <registryKey> set HOOKS_EDIT_ALLOWED 1` 명령으로 설정/해제)
"""
...
def main() -> None:
    ...
    if tool_name == "Bash":
        ...
        # .claude.workflow/workflow/bypass 참조 여부에 따라 차단 메시지 분기
    ...
    # .claude.workflow/workflow/bypass 경로 포함 여부 검사
    if ".claude.workflow/workflow/bypass" in file_path:
        # bypass 파일은 환경변수 우회 불가 (무조건 차단)
        _deny(HOOKS_BYPASS_FILE_DENIED)

    # .claude.workflow/hooks/ 경로 포함 여부 검사
    if ".claude.workflow/hooks/" in file_path:
        # 환경변수 우회 검사
        if hook_edit_allowed in ("true", "1"):
            sys.exit(0)
    ...
    # .claude.workflow/hooks/ 경로 미매칭 시 통과
    sys.exit(0)
