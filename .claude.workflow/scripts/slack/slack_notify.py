...
새 시그니처 (workDir 기반):
    ...
    - workDir이 .claude.workflow/ 또는 절대경로(/)로 시작하면 새 방식으로 감지
    - workDir 형식: .claude.workflow/workflow/YYYYMMDD-HHMMSS/<workName>/<command>
...
def main() -> None:
    ...
    # 이중 시그니처 감지
    if args and (args[0].startswith(".claude.workflow/") or args[0].startswith("/")):
