#!/usr/bin/env python3
"""ensure_bin_path.sh - Claude Code Bash tool 환경에 .claude.workflow/bin PATH를 주입한다.
...
def main() -> None:
    """CLAUDE_ENV_FILE에 .claude.workflow/bin PATH export를 작성한다.
    ...
    if not project_dir:
        ...
        # ensure_bin_path.sh는 .claude.workflow/scripts/session_start/ 에 위치
        # 따라서 ../../../../ = project root
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_dir = os.path.normpath(os.path.join(script_dir, '..', '..', '..'))

    bin_dir = os.path.join(project_dir, '.claude.workflow', 'bin')
