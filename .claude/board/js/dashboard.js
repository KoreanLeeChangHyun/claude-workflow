...
/**
 * Fetches all dashboard files in parallel and caches in Board.state.dashData.
...
function fetchAllDashboardFiles() {
  return Promise.all(DASH_FILES.map(fetchDashboardFile)).then(function (results) {
    DASH_FILES.forEach(function (name, i) { Board.state.dashData[name] = results[i]; });
