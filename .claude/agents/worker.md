...
```mermaid
flowchart TD
    S1[1단계: 계획서에서 작업 목록 확인] --> S2[2단계: 스킬 로드]
    S2 --> S3[3단계: 작업 진행]
...
작업 내역 파일에 반드시 "로드된 스킬" 섹션을 포함하여, 어떤 스킬을 로드했는지와 매칭 근거를 기록합니다.
...
| 스킬명 | 매칭 방식 | 근거 |
|--------|----------|------|
| [스킬명] | skills 파라미터 / 명령어 기본 / 키워드 판단 / description 폴백 | [구체적 근거] |
```

- **매칭 방식 분류**: `skills 파라미터` (오케스트레이터가 명시 전달), `명령어 기본` (command-skill-map.md 매핑), `키워드 판단` (태스크 내용 키워드 매칭), `description 폴백` (SKILL.md description 필드 기반 매칭)
- 스킬을 로드하지 않은 경우에도 "로드된 스킬 없음"과 그 사유를 기록
- 이 섹션은 "변경 파일" 섹션 바로 다음, "수행 내용" 섹션 앞에 배치
