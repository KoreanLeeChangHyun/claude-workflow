...
| 스킬 | 유형 | 바인딩 방식 | 용도 |
|------|------|------------|------|
| `workflow-agent-validator` | Tier 1 (에이전트) | frontmatter `skills` | MVP 검증 절차, 조건부 스킵 로직, 결과 판정 규칙, 검증 내역 작성 규격 |
| 전문화 스킬 | Tier 2 (전문화) | 계획서 스킬 컬럼 명시 시 동적 로드 | 특수 검증 도메인 (보안 검증, 성능 검증 등) |

> Validator는 기본적으로 `workflow-agent-validator` 스킬만 frontmatter에 정적 바인딩합니다. 단, 계획서 스킬 컬럼에 validator 전용 스킬이 명시된 경우 동적 로드 가능합니다.
>
> **Note**: 현재는 `workflow-agent-validator` 단독 운용이 기본이지만, 향후 보안 검증(`security-audit`), 성능 검증(`performance-check`) 등의 전문화 스킬을 계획서 스킬 컬럼으로 로드하는 확장이 가능합니다.
...
1. **계획서 로드 및 요구사항 파악** - `planPath`에서 계획서를 Read하여 태스크 목록(W01, W02, ...) 및 검증 범위 파악
2. **선행 산출물 참조** - `<workDir>/work/W*-*.md` 전체를 Glob으로 탐색하고 각 파일을 Read하여 검증 컨텍스트 확보 (어떤 파일이 변경되었는지, 어떤 의존성이 있는지 파악)
3. **환경 감지** - 프로젝트 루트에서 검증 도구 설정 파일 존재 여부를 확인하여 실행 가능한 검증 항목을 결정
4. **검증 실행** - 감지된 검증 도구(린트, 타입체크, 빌드)를 순차 실행. 미감지 항목은 SKIP. 작업 내역 파일 존재 여부도 확인
...
2. **산출물 필수 생성**: 모든 검증 태스크는 반드시 `work/validation-report.md`를 생성해야 한다. 검증 결과가 전체 SKIP이더라도 파일은 반드시 생성
...
8. **선행 산출물 참조 필수**: `<workDir>/work/W*-*.md` 전체를 Glob으로 탐색하고 각 파일을 Read하여 검증 컨텍스트를 확보한다
