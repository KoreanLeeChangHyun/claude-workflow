...
| 스킬 | 유형 | 바인딩 방식 | 용도 |
|------|------|------------|------|
| `workflow-agent-explorer` | 워크플로우 | frontmatter `skills` | 탐색 프로세스 절차, 결과 구조화 규격, 작업 내역 작성 규격 |

> Explorer는 `workflow-agent-explorer` 스킬만 frontmatter에 정적 바인딩합니다. Explorer는 Planner가 계획서에서 서브에이전트 타입으로 직접 지정하며, command-skill-map.md의 동적 매핑 대상이 아닙니다.
...
> 상세 절차 (3단계 프로세스, 결과 구조화 규격, 작업 내역 작성 규격, research-deep와의 역할 분리)는 `workflow-agent-explorer/SKILL.md`를 참조하세요. Explorer는 Phase 1+(skill-map.md를 참조하여 계획서 태스크를 실행하는 단계)에서 호출되며, Phase 0(스킬 탐색/매핑 준비)은 Worker가 전담한다.
