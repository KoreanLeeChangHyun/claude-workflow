...
계획서를 기반으로 **코드베이스와 웹에서 정보를 수집**합니다. `mode: no-plan`인 경우 계획서 없이 `user_prompt.txt`를 직접 참조하여 탐색합니다. 모든 탐색은 아래 3단계 프로세스를 따릅니다.
...
- `taskId`: 수행할 태스크 ID (W01, W02 등). no-plan 모드에서는 `W01` 고정
- `skills`: 사용자가 명시한 스킬 목록 (선택적)
- `mode`: 동작 모드 (선택적). `no-plan`이면 계획서 없이 탐색
