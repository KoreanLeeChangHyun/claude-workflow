...
- `python3 .claude/scripts/workflow/update_state.py` 상태 전이 (WORK -> REPORT)
...
1. **세션 링크 등록** - `python3 .claude/scripts/workflow/update_state.py link-session <registryKey> "${CLAUDE_SESSION_ID}"` 실행 (실패 시 비차단)
