...
> **책임 경계**: history.md 갱신, status.json 완료 처리, 사용량 확정, 레지스트리 해제, DONE 배너는 오케스트레이터가 flow-finish + flow-claude end로 직접 수행합니다.
...
1. **세션 링크 등록** - `.claude/scripts/flow/update_state.py link-session <registryKey> "${CLAUDE_SESSION_ID}"` 실행 (실패 시 비차단)
2. **템플릿 로드** - `.claude/skills/workflow-agent-reporter/templates/` 에서 command별 템플릿 Read 로드 (필수)
...
> **금지 항목**: 요약, 태스크 수, 변경 파일 수, 다음 단계, 경로, 메타정보(N개) 등 추가 정보 일체 금지
...
4. **보고서 생성 + summary.txt 생성 담당**: reporter의 담당 범위 (후속 마무리 작업은 오케스트레이터가 flow-finish로 수행)
