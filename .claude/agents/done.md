...
2. **status.json 완료 처리** - `step-update status <registryKey> REPORT DONE|FAILED` 실행. noreport 모드에서는 REPORT 단계가 없으므로 `step-update status <registryKey> WORK DONE|FAILED` 실행 (from_phase가 WORK)
3. **사용량 확정** - 성공 시 `step-update usage-finalize <registryKey>` 실행 (실패 시 비차단)
4. **레지스트리 해제** - `step-update unregister <registryKey>` 실행
