...
if [ "$PHASE" = "DONE" ] && [ -n "$STATUS" ]; then
    ...
    # .done-marker 생성: Stop Hook이 DONE 이후 block하지 않도록 방어
    touch "${PROJECT_ROOT}/.workflow/.done-marker"
    ...
    # ─── 시작 배너 ───
    # INIT 시작 시 이전 워크플로우의 .done-marker 정리
    if [ "$PHASE" = "INIT" ]; then
        rm -f "${PROJECT_ROOT}/.workflow/.done-marker"
    fi
    echo ""
