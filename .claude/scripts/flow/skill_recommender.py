...
def parse_skill_descriptions(catalog_path):
    """
    skill-catalog.md의 Skill Descriptions 섹션에서 스킬별 Triggers 키워드를 파싱.

    Returns:
        dict: {skill_name: [trigger_keyword, ...]}
    """
...
def tokenize(text):
    """텍스트를 소문자 토큰 리스트로 분할. 한글/영문 혼합 지원."""
    text = text.lower()
    # 영문 단어 추출
...
def build_tfidf_index(skills):
    """
    스킬별 Triggers + description 단어를 기반으로 TF-IDF 인덱스를 구축.

    Returns:
        dict: {skill_name: {term: tfidf_score}}
        dict: {term: idf_score}
    """
    num_docs = len(skills)
    if num_docs == 0:
...
def keyword_match_boost(query_text, skill_info):
    """
    Trigger 키워드가 쿼리에 직접 포함된 경우 추가 점수 부여.
...
def recommend(query_text: str, catalog_path: str | None = None) -> list[tuple[str, float]]:
    """
    태스크 description을 받아 상위 K개 스킬을 추천.

    Args:
        query_text: 태스크 설명 문자열
        catalog_path: skill-catalog.md 경로 (None이면 기본값)

    Returns:
        list: [(skill_name, score), ...] 상위 K개
    """
...
def print_help():
    """사용법 출력."""
    print("skill_recommender.py - TF-IDF 기반 스킬 자동 추천")
    print()
    print("사용법:")
    print("  python3 skill_recommender.py <task_description>")
    print("  python3 skill_recommender.py --help")
    print()
    print("설명:")
    print("  skill-catalog.md의 Skill Descriptions 섹션에서 각 스킬의")
    print("  Triggers 키워드를 파싱하여 인메모리 인덱스를 구축한 후,")
    print("  입력 태스크 description과 TF-IDF 유사도를 계산하여")
    print("  상위 3개 스킬을 추천합니다.")
    print()
    print("예시:")
    print('  python3 skill_recommender.py "보안 리뷰 및 OWASP 취약점 분석"')
    print('  python3 skill_recommender.py "React 컴포넌트 성능 최적화"')
    print('  python3 skill_recommender.py "GitHub Actions CI 빌드 실패 디버깅"')
    print()
    print("출력:")
    print("  추천 스킬 3개 (이름 + 유사도 점수)")
