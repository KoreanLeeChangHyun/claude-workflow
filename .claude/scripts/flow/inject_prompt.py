...
동작:
  ...
  - 메인 세션에서 .claude/prompt/includes/ 디렉터리의 모든 .xml 파일을 알파벳 순으로
...
def _load_includes(project_root: str) -> str:
    ...
    .claude/prompt/includes/ 디렉터리 내 모든 .xml 파일을 파일명 알파벳 순으로
    ...
    """
    includes_dir = os.path.join(project_root, ".claude", "prompt", "includes")
