...
- **어시스턴트 검증 안 된 주장 부풀리기 회피 (메타 MUST)**: 옵션 평가 시 측정·근거 없는 주장 부풀리지 말 것. "토큰 효율 / 정확성 / 비용 절감 / Claude 처리와 사용자 양쪽 모두 이득" 식 양쪽 편향 주장 회피. 측정·근거 없으면 명시 ("정량 근거 없음" / "추정"). 사용자 회의 신호 (`?`, `~ 아님`, 단답 의문) 시 즉시 자세 낮추고 검증. 티켓 생성 가치 평가 시 "정말 티켓 생성할 가치가 있는가?" 1차 자가 검증 — 사용자 영향 없으면 그냥 둠.
- **자기 검증 결함 인지 (메타)**: 룰 추가만으론 일관 적용 보장 불가. 어시스턴트가 매 응답마다 출력 자가 검증 단계 안 거치면 룰 위반 재발. 룰 우선순위·schema 차이가 아니라 자기 검증 의존성이 본 캐논의 한계.
...
- 재기동 2회 (계측 추가 → fix 검증) 가 정석. workflow.md / board.md §5 "한 번에 모아서 처리" 룰보다 **TDD 가 상위** — fix 자체에 대한 한 번에 모아서 처리는 유효, 진단 단계와 fix 단계 분리는 TDD 우선
...
### 사례 (2026-05-12, ESC 새로고침 busy 회귀)
클라 디버그 로그로 origin 후보 2개 (server 측 `awaiting_response` + `pending_turn`) 좁힘. 사용자 "맞습니까?" 회의 신호 → server 측 6지점 (`_awaiting_response` 변경 시점·process 종료·status 응답·pending_turn 판정 입력) 계측 추가. 코드 결정론으로 단정 가능했어도 사용자 룰에 따라 fix 전 실측 확보 단계 거침.
...
- 어시스턴트→사용자 출력에 "티켓 생성" 표현 사용 금지 (MUST NOT). 작업 진행 의향을 물을 때는 "티켓 생성 진행할까요" / "룰 추가할까요" / "메모리 등록할까요" 등 구체 작업 어휘로 대체. 단 사용자 발화 트리거 키워드(`티켓 생성해줘` 등)는 grill-me 호출 트리거로 그대로 보존 — 양방향 구분 (사용자→어시스턴트 트리거 어휘 보존 / 어시스턴트→사용자 출력 어휘 교체)
...
- **표현 명확성**: 비전문 표현으로 사용자 혼선 유발 회피 (위 응답 스타일의 "앞선다/뒤처진다" 룰 참조).
...
| 워크플로우 진행 여부 | `flow-sessions` + `ls .claude-organic/runs/` |
| 서버 재기동 여부 | `.pyc` mtime + `.board.url` mtime + backend live 호출 (PID/lstart 단독 X) |
| 코드 활성 여부 | grep + ls + cat (메모리 라인 번호도 실측) |

**Review 도달 ≠ 작업 완료**: finalize 가드 사각지대 (work/+report만 검증) 존재. 워크트리 commits ahead 동시 검증 의무.
...
- **모든 destructive 명령**: 사용자 명시 동의 필수. 진단 의도라도 backend write 호출 (`/api/kanban/done` 등) = 결과적 머지로 이어진 사례 (T-417). read-only 진단만 자율.
- **Board 서버 재기동/push/PR 머지**: 사용자 동의 (`푸시하세요` / `재기동` / Restart 버튼) 받기 전까지 자의적 진행 금지.

### 메모리 정확성 (Self-Discipline)
- 메모리 부정확 누적 → 답변 품질 저하. 메모리 티켓 생성 = 미래 답변의 기반 → 정확성 절대 룰.
...
### Agent Role 어휘 분리 (동음이의어 회피)

| 어휘 | 의미 | 실체 |
|------|------|------|
| **워커 (worker)** | 서브에이전트 워커 | Task 도구로 호출되는 SDK 서브에이전트. WORK 단계 수행 (worker-opus / worker-sonnet / explorer-file-sonnet 등) |
| **오케스트레이터 (orchestrator)** | 워크플로우 엔진의 메인 | `/wf` 진입점 주체 = 사용자가 직접 대화하는 메인 세션 (Claude Code 본체). INIT/PLAN/WORK/VALIDATE/REPORT/DONE 흐름 제어 + Task 호출 |
| **planner / reporter** | PLAN / REPORT 단계 Task 에이전트 | 별도 단계 전용, worker 와 구분 |
| **validator / verifier** | VALIDATE 단계 rule-based 엔진 | 에이전트 아님 |

- INIT 단계 행동 주체 = 오케스트레이터 (메인 세션). "워커가 INIT 단계 수행"은 오기.
- "워커 commit 누락" = WORK 단계 Task 에이전트 commit 누락.
- 코드/SKILL.md/보고서 어디서든 어휘 혼용 발견 시 즉시 분리.
