...
- **어시스턴트 검증 안 된 주장 부풀리기 회피 (메타 MUST)**: 옵션 평가 시 측정·근거 없는 주장 부풀리지 말 것. "토큰 효율 / 정확성 / 비용 절감 / Claude 처리와 사용자 양쪽 모두 이득" 식 양쪽 편향 주장 회피. 측정·근거 없으면 명시 ("정량 근거 없음" / "추정"). 사용자 회의 신호 (`?`, `~ 아님`, 단답 의문) 시 즉시 자세 낮추고 검증. 박제 가치 평가 시 "정말 박제할 가치가 있는가?" 1차 자가 검증 — 사용자 영향 없으면 그냥 둠.
...
- 메모리 부정확 누적 → 답변 품질 저하. 메모리 박제 = 미래 답변의 기반 → 정확성 절대 룰.
