...
> 2026-04-22 `.claude.workflow/` → `.claude-organic/`, `kanban/` → `tickets/`, `scripts/` → `engine/` rename 후 외부 프로젝트에서 누적된 stale 회귀를 일괄 잡으며 정착된 패턴.
...
실제 회귀 (2026-04-27): settings.json 의 `env` 블록 자체가 어느 시점에 누락 → `flow-kanban: command not found` (Exit 127). PATH 첫 항목이 stale `.claude.workflow/bin` 이었음.
...
preserve_dirs=("tickets" "runs" "roadmap" "memo")  # 옛: kanban/workflow/dashboard/edit
preserve_files=(".settings" ".env" ".version" ".board.url" "build.url" ".last-session-id")
```

새 사용자 데이터 디렉터리 추가 시 (예: 향후 `captures/`) init script 갱신 필요.
...
**Why**: 사용자 피드백 "너무 초록 초록한데" (2026-04-27). ✓ 다발 → 시각 단조롭고 핵심 묻힘.
...
구현: `build.sh` (`print_success` CYAN), `init-claude-workflow.sh` (✓ 라인 GREEN→CYAN sed 일괄 치환). 헤더는 GREEN 보존.

## 8. `.claude/` 파일 수정 차단은 Anthropic 하드코딩

**Why**: Claude Code 내부 하드코딩 보호 (2026-04-22 사용자 명시 "hook 이랑 무관하게 엔트로픽이 맞는 거라서요"). 로컬 PreToolUse hook 비활성화로 우회 불가.
...
- BASH_ENV 우회 옵션 B 합의 시 후속 5단계: bash_env.sh 생성 → settings.json env.BASH_ENV 박기 → init script 가드 → settings.tmpl 동기화 → CLAUDE.md/workflow.md 표기 점검

## 관련 PR 시퀀스 (참고)

#28 develop→main 통합 / #29 build.sh defaults.conf 경로 / #30 .gitignore 자동 등록 / #31 옛 디렉터리명 전수 정합화 / #32 Board URL webroot 제거 / #33 좀비 서버 자동 종료 / #34 Build URL 복사 버튼 / #35 색상 위계 / #36 `_merge_json_keys` stale 정합화 / #37 `generate_board_url()` 두 URL / #38 Memory 버튼 분기.
