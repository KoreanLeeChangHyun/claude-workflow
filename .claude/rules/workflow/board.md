...
Board 가 발사하는 SSE 채널은 책임이 다른 3 종류로 분리되어 있다 — 각 채널의 이벤트 계약을 분리 박제한다 (T-497).
...
| workflow_step | 유일한 FSM 전이 경로 (`{step, prev_step, trigger, phase?, mode?, result?}`) | `terminal_channel.emit_step` (메인) + REST `/terminal/workflow/step` POST |

새 이벤트 추가 시 위 계약과 정합해야 한다. `type: "attachment"` 같은 미분류 갭은 별 트랙 티켓 생성 후 처리.

**Replay 정책 (T-497 결정)**: 새로고침 시 SSE 링버퍼 재생은 사용하지 않고 REST `/terminal/workflow/history` 가 단일 출처. `session.js:_injectRestHistory` 가 `_isReplaying = true/false` 토글 + 이벤트 dispatch + `Board.phaseTimeline.render()` + `fetchStatus()` 까지 수렴. 옛 `replay_start` / `replay_end` SSE 이벤트는 서버 발사 0건 + REST 단일화 결정으로 폐기됨 — board.md §1 표 + `session.js` listener 모두 제거.
...
새 per-session 채널이 추가될 때 (예: v3 driver) 부분 통합 가치 재평가. 본 시점에서는 통합 비용 (호출자 sweep + payload 모양 동질화) > 정합화 이득.
