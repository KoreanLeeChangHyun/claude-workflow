...
| `{{request}}` | 사용자 원문 (request 파라미터 그대로 기록) | request 파라미터 그대로 기록 | `로그인 기능 추가해주세요` |

**참고**: 제목(`# [제목]`)은 planner가 요청을 분석하여 적절한 제목을 자유롭게 생성합니다. placeholder가 아닌 가이드입니다.

**품질 레벨 (선택적 메타 필드)**: 명령어별 기본 Level과 다른 품질 수준이 필요할 때만 `- 품질 레벨: Level N` 형식으로 메타 정보에 추가합니다. 기본값은 `workflow-agent/reference/planner/quality-levels.md`를 참조하세요.
...
| 2 | 메타 정보 | 필수 | - | - |
| 3 | 사용자 질의응답 내역 | 필수 | - | - |
| 4 | 작업 요약 | 선택 | T1 태스크 1개만 있는 단순 계획, 또는 메타 정보의 "사용자 원문"으로 충분히 이해 가능한 경우 | goal 재서술 금지. HOW(기술적 접근 방식)만 기술 |
| 5 | 작업 흐름도 | 권장 | 워커 1명이거나 종속성 없는 단순 병렬 구조 | mermaid `flowchart TD` 형식 |
| 6 | 작업 목록 | 필수 | - | - |
| 7 | 워커별 작업 상세 | 권장 | 워커 1명이고 작업 단순 | H3 서브섹션 형식, 항목 4-5개, 행위 동사 시작 |
| 8 | 실행 계획 | 필수 | - | - |
| 9 | 기술 검증 기준 | 권장 | 단일 T1 태스크만 있는 매우 단순한 계획 | 측정 가능성 원칙 준수. 기술적 검증 기준만. `<criteria>` 재서술 금지 |
...
| research | 현황 요약, 배경 정보, 선행 연구 참조. **"연구 범위 경계"는 "작업 범위" 섹션으로 대체** | 현황 스냅샷 수치: 크기, 파일 수, 기간 등 (예: "총 2.3MB, 301개 파일, 36개 디렉터리, 기간 2일") |
...
기술 검증 기준은 측정 가능하게 작성해야 한다. 아래 좋은 예/나쁜 예를 참고하여 모든 기술 검증 기준에 정량적 수치, 구체적 파일명, 또는 검증 가능한 조건을 포함한다.
