...
> SHA는 7자 이상 40자 이하의 git short hash. commit을 수행하지 않은 경우 `없음` 기재. **금지 항목**: 작업 상세, 변경 파일 목록, "다음 단계" 안내 등 규격 외 추가 정보를 반환에 포함하지 않는다.
...
flowchart TD
    ...
    S3 --> S3_5["3.5. 자체 리뷰"]
    S3_5 --> S4["4. 실행 내역 작성"]
    S4 --> S5["5. git commit #lpar;코드/문서 변경 시 필수#rpar;"]
```

**5. git commit (코드/문서/설정 변경 시 필수):** 워크트리에서 변경이 발생한 경우, 작업 내역 파일 작성 후 종료 직전에 반드시 `git -C <worktree> add -A && git -C <worktree> commit -m "<요약>"`로 commit한다. commit 누락은 finalize 이후 워크트리에 미커밋 변경이 남는 회귀를 유발한다. **advisory only** — 누락 시 사용자가 `flow-merge --force` 또는 Board UI 1클릭 commit으로 수습 가능.
