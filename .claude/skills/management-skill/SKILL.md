...
### CSO 원칙 (Compressed Skill Optimization)

> **강제 조건**: 환경변수 `ENFORCE_CSO_PRINCIPLE=true` 시 적용. false 시 권고 수준으로 완화.

LLM은 스킬 로딩 시 description만 읽고 SKILL.md 본문을 스킵하는 경우가 발생합니다. 이는 description에 충분한 정보가 있다고 판단할 때 일어나며, 본문의 중요한 지침이 실행되지 않는 원인이 됩니다.

**description 작성 규칙:**

- description은 "Use when..." 트리거 조건만으로 제한합니다. 스킬이 언제 활성화되어야 하는지만 기술합니다.
- 워크플로우 요약, 프로세스 설명, 단계별 절차를 description에 포함하지 않습니다. 이 정보는 반드시 본문에만 위치해야 합니다.
- description이 길어질수록 LLM이 본문을 로드하지 않을 가능성이 높아집니다. 간결한 트리거 조건만 유지하세요.

**위반 예시 (금지):** "Executes a 5-step workflow: (1) analyze, (2) plan, (3) implement, (4) verify, (5) report..."

**올바른 예시:** "Use when implementing code changes, features, or bug fixes requested in a plan."
...
> **강제 조건**: 환경변수 `ENFORCE_TOKEN_EFFICIENCY=true` 시 적용. false 시 권고 수준으로 완화.

스킬 유형별 SKILL.md 본문의 단어 수 목표값:

| 스킬 유형 | 목표값 | 기준 |
|-----------|--------|------|
| 워크플로우 시스템 스킬 | <150 단어 | 절차만 기술, 설명 최소화 |
| 표준 전문화 스킬 | <200 단어 | 핵심 워크플로우 + 주요 결정 기준 |
| 레퍼런스 스킬 | 본문 <500 단어 + `references/` 분리 | 상세 내용은 참조 파일로 위임 |

목표값 초과 시 우선적으로 적용할 전략:
1. 중복 설명 제거 (Claude가 이미 아는 내용)
2. 상세 내용을 `references/` 파일로 이동 (→ `### 점진적 공개 설계 원칙` 참조)
3. 예시를 간결한 코드/테이블로 압축
...
SKILL.md 본문을 핵심 내용으로 유지하고 500줄 미만으로 제한하여 컨텍스트 증가를 최소화하세요. 단어 수 기준은 `### 토큰 효율 목표값` 섹션을 참조하세요. 이 한계에 근접하면 내용을 별도 파일로 분리하세요. 내용을 다른 파일로 분리할 때, SKILL.md에서 해당 파일을 참조하고 언제 읽을지 명확히 설명하는 것이 매우 중요합니다.
