...
## command-code-quality-checker PERF- 체계와의 관계

command-review-performance는 command-code-quality-checker의 PERF- 접두사 체계를 확장하는 전문 리뷰 체크리스트다.

| 항목 | command-code-quality-checker (PERF-) | command-review-performance |
|------|--------------------------------------|---------------------------|
...
| 스킬 | 용도 | 경로 |
|------|------|------|
| command-code-quality-checker | PERF- 접두사 기본 성능 메트릭 산출 | `.claude/skills/command-code-quality-checker/SKILL.md` |
| command-requesting-code-review | 리뷰 체크리스트 및 이슈 분류 기준 | `.claude/skills/command-requesting-code-review/SKILL.md` |
