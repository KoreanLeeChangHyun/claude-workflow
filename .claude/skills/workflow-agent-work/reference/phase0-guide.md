...
> **역할 구분**: Phase 0은 독립 indexer 에이전트가 스킬 카탈로그(`skill-catalog.md`)를 참조하여 계획서 태스크에 적합한 스킬을 매핑하고 skill-map.md를 생성하는 준비 단계이고, Phase 1+는 Worker가 skill-map.md를 참조하여 계획서 태스크를 실행하는 단계이다. Phase 0에서 스킬을 찾지 못하면 Phase 1+는 스킬 없이 진행한다.
...
> **Phase 배너**: 오케스트레이터는 Phase 0 indexer 호출 직전에 `Workflow <registryKey> WORK-PHASE 0 "phase0" sequential` 배너를 출력합니다. indexer 에이전트 자체는 Phase 배너를 호출하지 않습니다.
...
```mermaid
flowchart TD
    START[WORK 시작] --> P0[Phase 0 실행: indexer가 skill-map.md 생성]
...
Phase 0은 **1개 indexer 에이전트가 순차적으로** 실행합니다.

**Phase 0의 2가지 기능:**

1. **work 작업 디렉터리 생성**: `<workDir>/work/` 디렉터리를 명시적으로 생성합니다.

2. **스킬 매핑**: 스킬 카탈로그(`skill-catalog.md`)를 1회 Read하여 전체 스킬 정보를 획득하고, 4단계 매칭 우선순위(skills 파라미터 > 명령어 기본 매핑 > 키워드 매칭 > description 폴백)로 태스크별 스킬을 매핑합니다.
...
**Phase 0 완료 후:** Phase 0 완료로 준비 단계(스킬 매핑)가 종료되고, Phase 1+(작업 실행 단계)로 전환됩니다. 오케스트레이터는 skill-map.md를 참고하여 후속 Phase 1~N의 worker 호출 시 skills 파라미터를 전달합니다.

**Phase 0 실패 시 폴백:**

indexer 에이전트가 실행되었으나 실패를 반환한 경우, C 방식(개별 자율 결정)으로 자동 폴백합니다:
1. Phase 0(indexer) 실패를 로그에 기록
...
2. Phase 0 indexer를 호출했는가?
3. Phase 0 indexer의 반환값을 수신했는가?
...
|------|---------------------|--------------------------|
| **목적** | 스킬 카탈로그 기반 스킬 매핑 | 계획서 태스크 실행 |
| **산출물/입력** | 산출물: `skill-map.md` | 입력: `skill-map.md` |
| **실행 주체** | indexer 에이전트 | worker 에이전트 |
| **실행 방식** | 1개 indexer 순차 실행 | N개 worker 병렬/순차 실행 |
