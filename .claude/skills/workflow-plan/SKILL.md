...
- 요청: <request 파라미터의 원본 내용을 그대로 기록>
...
> **참고**: `복잡도`와 `서브에이전트` 컬럼은 선택적이며, 태스크가 단순하거나 모두 Worker 타입인 경우 생략 가능합니다.
...
> | 가정 사항 | 선택 | 모든 요구사항이 질의응답으로 100% 명확화된 경우 |
