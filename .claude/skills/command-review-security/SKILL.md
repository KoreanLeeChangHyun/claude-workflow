...
## command-static-analysis와의 역할 분리

| 영역 | command-static-analysis | command-review-security |
|------|------------------------|------------------------|
...
| 스킬 | 용도 | 경로 |
|------|------|------|
| command-static-analysis | 보안 도구 실행 및 심층 검사 (도구 기반 검증 위임) | `.claude/skills/command-static-analysis/SKILL.md` |
| command-code-quality-checker | SEC- 접두사 기본 보안 검사 (기본 계층 보안 이슈) | `.claude/skills/command-code-quality-checker/SKILL.md` |
| command-requesting-code-review | 리뷰 요청 체크리스트 (리뷰 프로세스 통합) | `.claude/skills/command-requesting-code-review/SKILL.md` |
