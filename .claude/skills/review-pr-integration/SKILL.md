...
description: "PR review integration skill that uses gh CLI to extract PR diffs, generate inline review comments, check CI status, and auto-verify checklists. Use when handling a pull request review request, performing final verification before PR merge, or automating PR diff analysis with structured comment generation."
license: "Apache-2.0"
---
...
## workflow-system-report-output 및 devops-github과의 역할 분리

| 항목 | workflow-system-report-output | devops-github | review-pr-integration |
|------|-----------|-------------------|----------------------|
...
**보완 관계:** workflow-system-report-output으로 PR을 생성한 뒤, review-pr-integration으로 해당 PR을 리뷰한다. devops-github은 두 스킬 모두에서 gh CLI 실행의 기반이 된다.
...
| 스킬 | 경로 | 관계 |
|------|------|------|
| workflow-system-report-output | `.claude/skills/workflow-system-report-output/SKILL.md` | PR 생성 시 제목/본문 생성 (작성자 관점) |
