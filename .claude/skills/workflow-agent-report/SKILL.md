...
1. **보고서 작성**
   1. command에 해당하는 템플릿 파일을 Read 도구로 로드
      ...
      - 템플릿 경로: `.claude/skills/workflow-agent-report/templates/<템플릿파일>`
...
> **Note**: reporter 반환 후, done 에이전트(Haiku)가 history.md 갱신, status.json 완료 처리, 사용량 확정, 레지스트리 해제, DONE 배너를 수행합니다. 상세 절차는 `workflow-agent-orchestration/step-done.md`를 참조하세요.
...
| [`.claude/skills/workflow-agent-work/SKILL.md`](.claude/skills/workflow-agent-work/SKILL.md) | 수정 | 링크 지침 추가 |
```
...
| 스킬 | 용도 | 경로 |
|------|------|------|
| devops-changelog | Git 커밋 기반 CHANGELOG/릴리스 노트 자동 생성 | `.claude/skills/devops-changelog/SKILL.md` |
| devops-pr-summary | 동적 컨텍스트 주입으로 PR 제목/요약 자동 생성 | `.claude/skills/devops-pr-summary/SKILL.md` |
| devops-verification | 작업 완료 선언 전 자동 검증 강제 | `.claude/skills/devops-verification/SKILL.md` |
