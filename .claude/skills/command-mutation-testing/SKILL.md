...
| 영역 | code-quality-checker | command-mutation-testing |
|------|---------------------|------------------------|
...
**상호 보완**: code-quality-checker가 코드 자체의 품질을 검사하고, command-mutation-testing이 해당 코드를 검증하는 테스트의 품질을 검사한다. 두 스킬이 동시 로드되면 코드 품질과 테스트 품질을 동시에 보장한다.
