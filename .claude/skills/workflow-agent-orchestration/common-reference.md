...
- **Workflow Skill** (`workflow-agent-*`, 8개): 워크플로우 단계 관리 및 오케스트레이션. 에이전트 frontmatter의 `skills:` 필드로 정적 바인딩.
- **Command Skill** (`<컬렉션>-<스킬명>`, 40+개): 개별 명령어의 구체적 기능 수행. `command-skill-map.md` 기반 동적 바인딩 (worker 전용).
...
flowchart TD
    ...
    REPORT_A -->|바인딩| WF_REPORT[workflow-agent-report]
    DONE_A -->|바인딩| WF_DONE[workflow-agent-done]
```
...
`<workDir>/status.json`으로 현재 단계와 전이 이력을 추적합니다. 스키마(9개 필드)는 workflow-agent-init skill 참조.
