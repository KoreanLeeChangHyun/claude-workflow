...
# validate-kanban.sh
# ==================
# 로드맵 진행 추적 파일과 roadmap.md 간의 정합성을 검증하고 프로젝트 완료 여부를 판단한다.
#
# 사용법:
#   validate-kanban.sh <kanban_board_path> <roadmap_path>
#
# 인자:
#   kanban_board_path  로드맵 진행 추적 파일 경로
#   roadmap_path       roadmap.md 파일 경로
#
# 정합성 검증 항목:
#   (a) roadmap.md의 마일스톤 ID 목록과 추적 파일의 마일스톤 ID 목록 일치 여부
#   (b) 워크플로우 ID 교차 검증 (GAP-V3: roadmap SSOT 기준 ERROR/WARN 분류)
#   (c) 상태 불일치 감지 (roadmap에 없는 WF가 추적 파일에 존재하는 경우 경고)
#   (d) GAP-V2: roadmap.md 마일스톤 완료 상태 교차 확인 (프로젝트 완료 조건 3)
#
# 프로젝트 완료 판단:
#   추적 파일의 Done 컬럼 마일스톤 수와 전체 마일스톤 수 비교
...
# --- 인자 검증 ---
if [[ $# -ne 2 ]]; then
    echo "error: 인자 2개 필요 (kanban_board_path, roadmap_path)" >&2
    echo "사용법: validate-kanban.sh <kanban_board_path> <roadmap_path>" >&2
    exit 1
fi
...
if [[ -n "$ONLY_IN_ROADMAP" ]]; then
    ...
    while IFS= read -r ms_id; do
        ERRORS+=("마일스톤 불일치: ${ms_id}가 roadmap에 있으나 칸반 추적 파일에 없음")
...
if [[ -n "$ONLY_IN_KANBAN" ]]; then
    HAS_ERROR=1
    while IFS= read -r ms_id; do
        ERRORS+=("마일스톤 불일치: ${ms_id}가 칸반 추적 파일에 있으나 roadmap에 없음")
    done <<< "$ONLY_IN_KANBAN"
fi
...
if [[ -n "$WF_ONLY_IN_ROADMAP" ]]; then
    ...
    while IFS= read -r wf_id; do
        ERRORS+=("워크플로우 누락: ${wf_id}가 roadmap에 있으나 칸반 추적 파일에 없음")
...
if [[ -n "$WF_ONLY_IN_KANBAN" ]]; then
    while IFS= read -r wf_id; do
        WARNINGS+=("상태 불일치: ${wf_id}가 칸반 추적 파일에 있으나 roadmap에 없음")
...
if [[ -n "$DONE_BUT_NOT_IN_ROADMAP" ]]; then
    while IFS= read -r ms_id; do
        WARNINGS+=("완료 상태 미기록: ${ms_id}가 칸반 추적 파일 Done에 있으나 roadmap.md에 완료 상태가 기록되지 않음")
