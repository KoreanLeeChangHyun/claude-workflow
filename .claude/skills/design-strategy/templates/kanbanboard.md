<!--
  ...
  =========================
  strategy 명령어가 roadmap.md 생성 직후 이 템플릿을 기반으로 .kanban/board.html 파일을 갱신한다.
  ...
  치환 포인트({{placeholder}}) 목록:
    ...
    - {{created_date}}  : .kanban/board.html 생성일 (YYYY-MM-DD)
    - {{updated_date}}  : .kanban/board.html 최종 갱신일 (YYYY-MM-DD)
  ...
  티켓 자동 생성 절차 (마일스톤 카드 내 워크플로우 항목):
    ...
    2. 티켓 번호(NNN)는 .kanban/board.html를 파싱하여 기존 최대 번호 + 1로 자동 채번한다.
    3. 생성된 티켓은 .kanban/board.html의 해당 마일스톤 카드 워크플로우 항목에
       `- [ ] T-NNN: {{wf_name}} ({{wf_command}})` 형식으로 기록한다.
    4. 티켓 파일 경로: .kanban/T-NNN.xml
       티켓 내용 형식: YAML frontmatter + XML 태그 본문 (T-NNN.xml 티켓 형식 규격 참조)

  SSOT 원칙:
    - roadmap.md = 계획 정보의 단일 진실 소스 (마일스톤 ID, 명칭, DoD, 워크플로우 체인)
    - .kanban/board.html = 실행 상태 전용 (컬럼 배치, 티켓 체크 상태, 완료일)
  ...
  티켓 항목 형식 (board.html 내 Open/In Progress/Done 컬럼):
    - 미완료: - [ ] T-NNN: 티켓제목 (command)
    - 완료:   - [x] T-NNN: 티켓제목 (command)

  티켓 컬럼 간 이동 규칙:
    - Open → In Progress: 워크플로우 시작 시 initialization.py가 board.html의 티켓 항목을 In Progress로 이동
    - In Progress → Done: 워크플로우 완료 시 finalization.py가 board.html의 티켓 항목을 Done으로 이동하고 [x]로 체크
-->
---
project: "{{project}}"
