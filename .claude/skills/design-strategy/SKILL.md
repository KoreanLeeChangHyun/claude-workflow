...
/wf -s (strategy) 명령어의 핵심 실행 엔진으로, 다음 활동을 체계적으로 수행한다:
...
- **T-NNN.xml 티켓 자동 생성**: roadmap.md 생성 직후 각 워크플로우에 대한 `.kanban/active/T-NNN.xml` 티켓 파일을 자동 생성하여 마일스톤별 실행 상태를 추적

## 복잡도 산정 가이드

워크플로우 단위의 복잡도를 산정하여 일정 예측과 리소스 배분의 근거를 제공한다. 태스크 단위 복잡도 산정(workflow-agent/reference/planner-guide.md 참조)을 워크플로우 수준으로 확장한 것이다.
...
/wf -s (strategy) 명령어의 최종 산출물인 `roadmap.md`의 표준 구조. strategy.md의 "핵심 산출물" 섹션과 정합성을 유지한다.
...
roadmap.md 생성 직후 각 워크플로우에 대한 `.kanban/active/T-NNN.xml` 티켓 파일을 자동 생성한다. 로드맵 진행 상태는 XML 티켓 파일을 직접 스캔하여 관리한다.
...
| .kanban/active/T-NNN.xml | 워크플로우별 티켓 | 워크플로우 명령어, 목표, 대상, 제약사항, 완료 기준, 실행 상태 |

- roadmap.md의 마일스톤 ID/명/DoD를 기반으로 워크플로우별 티켓을 생성한다
- 실행 상태 추적은 `.kanban/active/` 디렉터리의 XML 파일을 직접 스캔하여 수행한다
...
2. **채번**: `.kanban/active/` 디렉터리의 XML 파일을 스캔하여 기존 최대 티켓 번호(T-NNN) + 1로 자동 채번한다 (중복 방지)
...
strategy 재호출 시(`.kanban/active/` 디렉터리에 XML 티켓 존재) 실행되는 Judge 모드의 상세 로직을 기술한다.
...
| workflow-agent | 개별 워크플로우 내부의 태스크 분해 및 Phase 설계 | 워크플로우별 상세 계획 수립 시 참조 |
