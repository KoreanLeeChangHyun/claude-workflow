---
name: architect
description: 시스템 아키텍처 설계 및 UML 다이어그램 생성. 사용자가 "시스템 설계해줘", "아키텍처 다이어그램 만들어줘", "UML 그려줘" 등을 요청할 때 사용. 클래스 다이어그램, 시퀀스 다이어그램, ER 다이어그램, 컴포넌트 다이어그램, 배포 다이어그램, 상태 다이어그램 등 모든 UML 다이어그램 생성 지원. Mermaid 코드(.md)와 이미지(.png) 모두 생성.
---
