...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const parseInlineFormatting = (element, baseOptions = {}, runs = [], baseTextTransform = (x) => x, validationErrors = []) => {
        ...
        if (isText) {
          ...
          if (node.tagName === 'SPAN' || node.tagName === 'B' || node.tagName === 'STRONG' || node.tagName === 'I' || node.tagName === 'EM' || node.tagName === 'U') {
            ...
            if (computed.marginLeft && parseFloat(computed.marginLeft) > 0) {
              validationErrors.push(`Inline element <${node.tagName.toLowerCase()}> has margin-left which is not supported in PowerPoint. Remove margin from inline elements.`);
            }
            if (computed.marginRight && parseFloat(computed.marginRight) > 0) {
              validationErrors.push(`Inline element <${node.tagName.toLowerCase()}> has margin-right which is not supported in PowerPoint. Remove margin from inline elements.`);
            }
            if (computed.marginTop && parseFloat(computed.marginTop) > 0) {
              validationErrors.push(`Inline element <${node.tagName.toLowerCase()}> has margin-top which is not supported in PowerPoint. Remove margin from inline elements.`);
            }
            if (computed.marginBottom && parseFloat(computed.marginBottom) > 0) {
              validationErrors.push(`Inline element <${node.tagName.toLowerCase()}> has margin-bottom which is not supported in PowerPoint. Remove margin from inline elements.`);
            }

            // Recursively process the child node. This will flatten nested spans into multiple runs.
            parseInlineFormatting(node, options, runs, textTransform, validationErrors);
          }
        }
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        liElements.forEach((li, idx) => {
          ...
          const runs = parseInlineFormatting(li, { breakLine: false }, [], (x) => x, errors);
