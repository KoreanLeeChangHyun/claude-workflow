...
- [ ] **`<constraints>` 제약 조건**: 기술적 제약 조건이 1개 이상 포함되어 있는가?
- [ ] **`<criteria>` 완료 기준**: 검증 가능한 완료 기준이 명시되어 있는가?
...
| # | 요소 | 설명 | 나쁜 예 | 좋은 예 | XML 태그 매핑 |
|---|------|------|---------|---------|---------|
| 1 | **명확한 목표** | 동사 포함, 구체적 행동 지시 | "확인해줘" | "validateEmail 함수를 구현하시오" | `<goal>` |
| 2 | **구체적 대상** | 파일, 모듈, 함수명 명시 | "코드 고쳐줘" | "`src/auth/session.ts`의 `refreshToken` 함수를 수정하시오" | `<target>` |
| 3 | **제약 조건** | 사용 가능/불가 기술, 패턴 한정 | (없음) | "외부 라이브러리 없이 표준 라이브러리만 사용" | `<constraints>` |
| 4 | **검증 기준** | 테스트 케이스 또는 성공 조건 | (없음) | "`user@example.com` -> true, `invalid` -> false로 검증" | `<criteria>` |
| 5 | **참조 컨텍스트** | 배경 정보, 기존 패턴 파일 경로 | (없음) | "`@src/auth/oauth.ts` 패턴을 참고하여 구현" | `<context>` + `<reference>` |
| 6 | **XML 태그 구조** | planner 핸드오프 시 구조화된 형식 제공 | (없음) | "<goal>목표</goal><target>대상</target><constraints>제약</constraints><criteria>기준</criteria>" | (구조화 형식) |

XML 태그는 5요소를 구조화하는 **형식**이며, 5요소 자체를 대체하지 않음.
...
### XML 태그 버전 (4-블록 분리 패턴)

```
## INSTRUCTIONS
<goal>최종 달성 목표를 한 문장으로 기술합니다</goal>

## CONTEXT
<context>배경 정보, 관련 파일, 기존 패턴을 정리합니다</context>
<reference>참고할 기존 구현체나 문서 경로를 명시합니다</reference>

## TASK
<target>작업 대상이 되는 파일, 모듈, 함수명을 명시합니다</target>
<constraints>사용 가능/불가 기술, 패턴의 제약 조건을 기술합니다</constraints>
<scope>작업 범위(In-Scope), 제외 사항(Out-of-Scope)을 명시합니다</scope>

## OUTPUT FORMAT & CRITERIA
<criteria>완료 기준이 되는 테스트 케이스, 성공 조건을 명시합니다</criteria>
```

**주의:**
- XML 태그 외부의 자유 텍스트도 함께 사용 가능 (하이브리드)
- 필수 태그: `<goal>`, `<target>`, `<constraints>`, `<criteria>`
- 선택 태그: `<context>`, `<scope>`, `<reference>`, `<approach>`
