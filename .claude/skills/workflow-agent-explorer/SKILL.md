...
```mermaid
flowchart TD
    S0[1단계: 요구사항 파악] --> Q1{종속 태스크?}
    Q1 -->|예| S0_5[2단계: 선행 결과 읽기]
    Q1 -->|아니오| SKILL
    S0_5 --> SKILL[3단계: 스킬 로드]
    SKILL --> S1[4단계: 탐색 실행]
    S1 --> S3[5단계: 작업 내역 생성]
```
...
계획서의 스킬 컬럼 또는 `skills` 파라미터를 기반으로 전문화 스킬을 로드한다.

**로드 우선순위:**

1. **skill-map.md 참조**: `<workDir>/work/skill-map.md`가 존재하면 자신의 `taskId` 섹션을 Read하여 스킬 지침 획득
...
계획서에서 탐색 주제를 추출하고 검색 쿼리를 생성한 후, 도구별로 실행한다.
...
## research-deep와의 역할 분리

| 항목 | research-deep | Explorer (이 스킬) |
|------|--------------|-------------------|
...
**선택 기준:**
- 워크플로우 외부에서 코드만 빠르게 스캔 -> research-deep
...
> 이 형식 외의 추가 정보(탐색 결과 상세, 발견 목록, 다음 단계 등)는 절대 포함하지 않습니다.
...
> **금지 항목**: 탐색 결과 상세, 발견 파일 목록, "다음 단계" 안내, 실행 로그, 경로, 메타정보(N개) 등을 반환에 포함하지 않습니다. 이러한 정보는 작업 내역 파일에만 기록합니다.
