...
- 변경 범위를 축소 서술하여 검증 필요성을 낮추려는 시도
- 이전 성공 사례를 현재 상황의 증거로 사용
- 다른 프로젝트의 통과 결과를 현재 증거로 사용
- 마감 압박을 검증 생략의 이유로 사용

## Rationalization Prevention

> **강제 조건**: 환경변수 `ENFORCE_RATIONALIZATION_GUARD=true` 시 적용. false 시 권고 수준으로 완화.
...
| "작은 변경이라 괜찮다" | 크기는 면제 조건이 아니다 (점진적 양보 방지) |
| "이전에도 이렇게 했다" | 이전 관행 != 현재 증거 (일관성 편향 방지) |
| "다른 프로젝트에서는 통과했다" | 다른 컨텍스트 != 현재 컨텍스트 (사회적 증거 방지) |
| "시간이 부족하다" | 시간 압박 != 검증 면제 (희소성 압박 방지) |

**이 규칙의 글자를 위반하는 것은 정신을 위반하는 것이다.** 변명의 형태가 새로워져도 "검증 없이 완료를 주장할 수 없다"는 원칙은 변하지 않는다.
