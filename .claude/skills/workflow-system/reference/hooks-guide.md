...
.claude-organic/scripts/                        # 실제 로직 스크립트
├── banner/                             # 배너 출력
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "python3 -u .claude-organic/scripts/sync/history_sync.py sync && python3 -u .claude-organic/scripts/sync/history_sync.py archive",
      "timeout": 30,
      "async": true
    }
  ]
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "python3 -u .claude-organic/scripts/hooks/session_start_system_prompt.py",
      "timeout": 5
    }
  ]
}
...
- **분기 조건**:
  - `TMUX_PANE` 환경변수 있고 세션 이름이 `T-` 접두사로 시작 → `.claude-organic/prompt/system-prompt-wf.xml`
...
| 도구 | 핸들러 | 스크립트 | 동작 | 모드 |
|------|--------|----------|------|------|
| Write, Edit | catalog_sync | `.claude-organic/scripts/sync/catalog_sync.py` | SKILL.md 변경 시 카탈로그 자동 갱신 | async |
...
| 핸들러 | 스크립트 | 동작 | 모드 |
|--------|---------|------|------|
| usage-tracker | `.claude-organic/scripts/sync/usage_sync.py` | 토큰 사용량 추적 | async |
...
1. **로직 스크립트 작성**: `.claude-organic/scripts/<적절한-디렉터리>/`에 Python 스크립트 생성
...
| 파일 | 역할 | 매칭 도구 |
|------|------|-----------|
| `.claude-organic/scripts/guards/hooks_self_guard.py` | hooks/scripts 자기 보호 | Write, Edit, Bash |
| `.claude-organic/scripts/guards/dangerous_command_guard.py` | 위험 명령어 차단 | Bash |

### 워크플로우 유틸리티

| 파일 | 호출 방식 | 용도 |
|------|----------|------|
| `.claude-organic/scripts/flow/update_state.py` | `flow-update` alias | 워크플로우 상태 관리 |
| `.claude-organic/scripts/flow/finalization.py` | `flow-finish` alias | 워크플로우 마무리 처리 |
| `.claude-organic/scripts/flow/reload_prompt.py` | `flow-reload` alias | 프롬프트 리로드 |
| `.claude-organic/scripts/flow/garbage_collect.py` | `flow-gc` alias | 좀비 워크플로우 정리 |
| `.claude-organic/scripts/banner/flow_claude_banner.sh` | `flow-claude` alias | 워크플로우 시작/종료 배너 |
| `.claude-organic/scripts/banner/flow_step_banner.sh` | `flow-step` alias | 스텝 시작/종료 배너 |
| `.claude-organic/scripts/banner/flow_phase_banner.sh` | `flow-phase` alias | WORK 페이즈 배너 |
| `.claude-organic/scripts/banner/flow_update_banner.sh` | `flow-update` alias | 상태 전이 시각화 배너 |
