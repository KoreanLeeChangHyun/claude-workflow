...
- **분기 조건**:
  - `TMUX_PANE` 환경변수 있고 윈도우 이름이 `T-` 접두사로 시작 → `.claude.workflow/prompt/system-prompt-wf.xml`
  - 그 외 → `.claude.workflow/prompt/system-prompt.xml`
...
| `.claude.workflow/hooks/post-tool-use.py` | PostToolUse | catalog_sync, tmux_cleanup |
