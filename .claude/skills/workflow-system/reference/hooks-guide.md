...
- **트리거**: 세션 시작/재개/컴팩트 시
- **동작**: tmux 윈도우 이름(T-* 여부)으로 세션 유형을 판별하여 대응하는 system-prompt XML 파일을 stdout에 출력 → 세션 컨텍스트에 자동 주입
- **동기**: async 미지정 (동기 실행으로 stdout이 컨텍스트에 주입되도록 보장)
- **분기 조건**:
  - `TMUX_PANE` 환경변수 있고 윈도우 이름이 `T-` 접두사로 시작 → `.claude/prompt/system-prompt-wf.xml`
  - 그 외 → `.claude/prompt/system-prompt.xml`
- **파일 없으면 무시**: 대상 XML 파일이 존재하지 않으면 exit 0으로 종료
...
| Bash | tmux_cleanup | 인라인(`post-tool-use.py` 내부) | `flow-claude end` 감지 시 tmux 윈도우 지연 kill | async |
...
2. **thin wrapper 작성**: `.claude/hooks/<event>.py` 플랫 파일 패턴으로 디스패처 생성
...
| `.claude/scripts/banner/flow_update_banner.sh` | `flow-update` alias | 상태 전이 시각화 배너 |

## 참고

- `.claude/hooks/` — thin wrapper Hook 스크립트
- `.claude/scripts/` — 실제 로직 스크립트
