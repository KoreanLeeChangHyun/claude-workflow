...
roadmap.md 생성 직후 동일 디렉터리에 `.kanbanboard` 파일을 자동 생성한다. 템플릿은 `command-strategy/templates/kanbanboard.md`를 사용한다.
...
4. **템플릿 치환**: `command-strategy/templates/kanbanboard.md` 템플릿의 placeholder를 치환한다
   - `{{project}}` -> 추출한 프로젝트명
...
| 스킬 | 용도 | 참조 시점 |
|------|------|----------|
| command-strategy | 워커용 커맨드 스킬 (레거시 참조) | kanbanboard 템플릿 경로 참조 |
