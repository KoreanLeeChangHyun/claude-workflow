...
## code-quality-checker 및 tdd-guard-hook과의 역할 분리

| 영역 | tdd-guard-hook | code-quality-checker | command-test-design |
|------|---------------|---------------------|---------------------|
| 범위 | TDD 원칙 준수 감시 | 코드 품질 정량 검사 | 테스트 설계 기법 기반 케이스 생성 |
| 관점 | 테스트 먼저 작성했는가 | 린트/타입 오류가 없는가 | 입력 도메인이 충분히 커버되었는가 |
| 산출물 | 경고/차단 | 품질 점수 | 테스트 케이스 + 테스트 코드 |
| 시점 | 구현 전 | 구현 후 | 테스트 설계 시 |

**상호 보완**: tdd-guard-hook이 테스트 우선 작성을 강제하고, command-test-design이 체계적 테스트 케이스를 제공하며, code-quality-checker가 최종 품질을 검증한다.
