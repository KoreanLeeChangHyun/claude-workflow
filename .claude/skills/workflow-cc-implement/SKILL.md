...
implement 명령어는 리팩토링 작업을 포함한다. 대상이 불명확한 경우 최근 리뷰 대상(`.workflow/<최신작업디렉토리>/report.md`)을 참조한다. 최신 작업 디렉토리는 `.workflow/` 하위 디렉토리를 생성일시(YYYYMMDD-HHMMSS) 기준 역순 정렬하여 첫 번째 항목을 선택한다.
...
| PNG 변환 | `mmdc -i <file>.md -o <file>.png` (mmdc 미설치 시 Mermaid 코드 블록만 포함한 .md 파일로 대체 생성한다) |
