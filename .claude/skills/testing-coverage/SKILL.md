...
| 영역 | code-quality-checker | testing-coverage |
|------|---------------------|--------------------------|
...
**상호 보완**: code-quality-checker가 기본 커버리지 임계값을 확인하고, testing-coverage가 diff coverage 게이트와 미커버 영역 분석을 수행. 두 스킬이 동시 로드되면 testing-coverage의 상세 결과를 code-quality-checker가 참조한다.
