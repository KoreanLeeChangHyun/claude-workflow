...
> **Agent-Skill Binding**
> - Agent: `worker-opus` (model: opus, maxTurns: 50) / `worker-sonnet` (model: sonnet, maxTurns: 50)
...
> | Category | Rule |
> |----------|------|
> | **Allowed calls** | worker-opus, worker-sonnet, explorer, validator, reporter 에이전트 + skill_mapper.py 스크립트만 호출 가능 |
...
|------|--------------------|-----------------------|
| 서브에이전트 타입 | `worker-opus` / `worker-sonnet` | `validator` |
