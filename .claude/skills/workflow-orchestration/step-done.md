...
```bash
# 1. 상태 전이 (REPORT → DONE)  # NOTE: flow-finish Step 1이 이미 DONE이면 전이를 스킵하므로 이중 전이 안전
flow-update status <registryKey> DONE

# 2. 마무리 스크립트 실행 (history.md, usage, archive, kanban, tmux cleanup 1차)
# <status>: "완료" 또는 "실패" (다른 값 사용 금지. "성공" 등은 무효)
# Step 5: TMUX_PANE + T-* 윈도우 조건 시 3초 지연 후 tmux 윈도우 백그라운드 kill
flow-finish <registryKey> 완료 --ticket-number <T-NNN> [--workflow-id <id>]

# 3. 종료 배너 출력 (Slack 알림 자동 수행) / PostToolUse hook이 2차 cleanup(5초 지연)으로 동작
...
5. **tmux 윈도우 백그라운드 지연 kill** — `TMUX_PANE` 설정 시 윈도우 이름이 `T-*` 접두사인 경우에 한해 `nohup sleep 3 && tmux kill-window` 를 비차단 백그라운드로 실행. 비tmux 환경 또는 T-* 미해당 윈도우는 스킵
