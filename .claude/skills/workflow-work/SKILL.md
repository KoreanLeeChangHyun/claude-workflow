...
```
작업 수신 → 계획서 로드 → 스킬 로드 → 요구사항 파악 → 실행 → 결과 반환
```

**1. 작업 수신:** 프롬프트에서 command, workId, taskId, planPath, skills 확인

**2. 계획서 로드:** planPath에서 계획서를 읽어 할당된 태스크 정보 파악

**3. 스킬 로드:** skills 파라미터 또는 키워드 분석으로 필요한 스킬 로드

**4. 요구사항 파악:** 작업 유형 판단
- **코드 작성**: 새 파일 생성 또는 기존 파일 수정
- **코드 분석**: 코드베이스 탐색 및 분석
- **파일 작업**: 파일 읽기, 쓰기, 검색
- **실행 작업**: 명령어 실행, 테스트 수행
- **조사 작업**: 정보 수집 및 정리

**5. 실행:** 사용 가능한 모든 도구 활용
- `Read`, `Write`, `Edit`: 파일 작업
- `Grep`, `Glob`: 검색
- `Bash`: 명령어 실행
- `Task`: 하위 작업 위임 (필요시)

**6. 결과 반환:** 작업 완료 후 구조화된 결과 반환
