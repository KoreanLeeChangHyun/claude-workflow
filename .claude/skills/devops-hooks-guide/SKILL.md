...
│   ├── devops-dangerous-guard.py      # -> scripts/guards/dangerous_command_guard.py
...
- **관련 스킬**: `workflow-agent-plan` (PLAN 단계에서 사용자 승인 대기 시)
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "python3 .claude/hooks/pre-tool-use/devops-dangerous-guard.py",
...
- **동기**: 도구 실행을 차단할 수 있음
- **스크립트**: `.claude/hooks/pre-tool-use/devops-dangerous-guard.py` (thin wrapper -> `.claude/scripts/guards/dangerous_command_guard.py`)
- **관련 스킬**: `devops-dangerous-guard`
...
| `.claude/hooks/pre-tool-use/devops-dangerous-guard.py` | `.claude/scripts/guards/dangerous_command_guard.py` | 위험 명령어 차단 | PreToolUse | Bash |
...
- `devops-dangerous-guard` 스킬 - 차단 패턴 상세 정보
