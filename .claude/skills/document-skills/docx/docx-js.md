...
| 항목 | 잘못된 예 | 올바른 예 |
|------|----------|----------|
| PageBreak | `new PageBreak()` 단독 사용 | `new Paragraph({ children: [new PageBreak()] })` |
| ShadingType | `ShadingType.SOLID` (검은 배경) | `ShadingType.CLEAR` |
| ImageRun | `type` 파라미터 누락 | `type: "png"` 필수 지정 |
| Line breaks | `new TextRun("Line1\nLine2")` | 별도 `Paragraph` 요소 사용 |

### ALWAYS (필수 패턴)

| 항목 | 설명 |
|------|------|
| Default font | `styles.default.document.run.font: "Arial"` |
| Custom styles | Arial 폰트 기반 스타일로 시각적 계층 구조 확립 |
| Table widths | `columnWidths` 배열 + 개별 셀 `width` 모두 설정 |
| Table margins | TABLE 레벨에서 설정 (셀별 반복 방지) |
| Table borders | `Table`이 아닌 `TableCell`에 적용 |
| TextRun | `Paragraph.children`에는 항상 `TextRun` 객체 사용 |

### NEVER (금지 패턴)

| 항목 | 설명 |
|------|------|
| Unicode bullets | `"• Item"` 대신 `LevelFormat.BULLET` numbering config 사용 |
| String "bullet" | `format: "bullet"` 대신 `format: LevelFormat.BULLET` 상수 사용 |
| TOC + custom style | HeadingLevel만 사용, custom style 병용 금지 |

### Numbering 주의사항

- **같은 reference** = 번호 연속 (1,2,3 → 4,5,6)
- **다른 reference** = 번호 재시작 (1,2,3 → 1,2,3)
- 별도 번호 목록마다 고유한 reference name 사용

### 단위

- **DXA** (twentieths of a point): 1440 = 1 inch
- Letter usable width: 9360 DXA (1" margins 기준)