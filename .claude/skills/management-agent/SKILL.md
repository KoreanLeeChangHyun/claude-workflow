...
**환경변수 확인:**
```bash
if [ -f ".claude.workflow/.env" ]; then
    SLACK_BOT_TOKEN=$(grep "^CLAUDE_CODE_SLACK_BOT_TOKEN=" .claude.workflow/.env | sed 's/^CLAUDE_CODE_SLACK_BOT_TOKEN=//')
    SLACK_CHANNEL_ID=$(grep "^CLAUDE_CODE_SLACK_CHANNEL_ID=" .claude.workflow/.env | sed 's/^CLAUDE_CODE_SLACK_CHANNEL_ID=//')
fi
```
