...
현재 `.claude/settings.json`에 등록된 Hook은 총 5개입니다: SessionStart 2개, PreToolUse 1개 (디스패처), PostToolUse 1개 (디스패처), SubagentStop 1개 (디스패처).
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "python3 -u .claude/scripts/hooks/session_start_system_prompt.py",
      "timeout": 5
    }
  ]
}
...
- **트리거**: 세션 시작/재개/컴팩트 시
- **동작**: tmux 윈도우 이름(T-* 여부)으로 세션 유형을 판별하여 대응하는 system-prompt XML 파일을 stdout에 출력함으로써 세션 컨텍스트에 자동 주입한다
- **동기**: async 미지정 (동기 실행하여 stdout이 컨텍스트에 주입되도록 보장)
- **스크립트**: `.claude/scripts/hooks/session_start_system_prompt.py`
- **분기 조건**:
  - `TMUX_PANE` 환경변수 있고 윈도우 이름이 `T-` 접두사로 시작 → `.claude/prompt/system-prompt-wf.xml` (워크플로우 세션용)
  - 그 외 (메인 세션 또는 비tmux 환경) → `.claude/prompt/system-prompt.xml` (메인 세션용)
- **파일 없으면 무시**: 대상 XML 파일이 존재하지 않으면 에러 없이 exit 0으로 종료
...
| 도구 | 핸들러 | 스크립트 | 동작 | 모드 |
|------|--------|----------|------|------|
| Write, Edit | catalog_sync | `sync/catalog_sync.py` | SKILL.md 변경 시 카탈로그 자동 갱신 | async |
| Bash | tmux_cleanup | 인라인(`post-tool-use.py` 내부) | `flow-claude end` 감지 시 tmux 윈도우 지연 kill (2차 안전장치) | async |

- **플래그 제어**: `.claude.env`의 `HOOK_CATALOG_SYNC` 환경변수로 핸들러 활성화/비활성화

#### SubagentStop Hook (디스패처)

##### 4. Subagent-stop 디스패처
...
1. **로직 스크립트 작성**: `.claude/scripts/<적절한-디렉터리>/` 에 Python 스크립트 생성
2. **thin wrapper 작성**: `.claude/hooks/<event>.py` 플랫 파일 패턴으로 디스패처 생성 (scripts/ 호출)
...
| `.claude/hooks/post-tool-use.py` | PostToolUse | catalog_sync, tmux_cleanup |
| `.claude/hooks/subagent-stop.py` | SubagentStop | usage_sync |
...
| `.claude/scripts/sync/catalog_sync.py` | 스킬 카탈로그 동기화 (PostToolUse Hook에서 자동 트리거) |
