...
**workflow-agent-explore의 역할:**
- 오케스트레이터(workflow-agent-orchestration)가 Task 도구로 explorer 에이전트를 호출
...
| 스킬명 | 매칭 방식 | 근거 |
|--------|----------|------|
| workflow-agent-explore | frontmatter skills | explorer 에이전트 기본 스킬 |
...
`disable-model-invocation: true`: Claude의 자동 스킬 호출을 차단하여 워크플로우 순서를 보장합니다. 이 플래그는 workflow-agent-explore 스킬에만 적용되며 제거하지 마세요.
...
| workflow-agent-work | Worker 작업 스킬 (작업 내역 규격 참조) | `.claude/skills/workflow-agent-work/SKILL.md` |
