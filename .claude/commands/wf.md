...
```
\033[36m[T-NNN]\033[0m : \033[33m[WF]\033[0m wf 통합 명령어 사용법
...
```
\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m 티켓 T-NNN을 생성했습니다.
```
...
```
\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m 이전 대화를 기반으로 다음과 같이 추론했습니다:
...
```
\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m 어떤 목적의 티켓을 생성할까요?
...
현재 프롬프트 상태를 반영한 안내와 개선 제안을 출력합니다 (접두사: \033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m). 사용자 입력을 기반으로 goal, target, constraints, criteria, context 정보를 개선합니다.
...
1. **헤더**: 접두사(`\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m`) + 제목
...
**출력 예시**:
```
\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m 로그인 리다이렉트 버그 수정
...
> `<command>` 값은 1-3에서 결정된 용도에 해당하는 값을 사용합니다. `flow-kanban add-subnumber`의 `--command` 인자에 동일 값을 전달합니다. constraints, criteria, context 값이 없는 경우 해당 인자를 생략합니다.
...
- 복원 완료 후 안내 메시지 출력:
  ```
  \033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m T-NNN 티켓을 Done에서 Open으로 복원했습니다.
  ```
...
```
\033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m T-NNN 티켓을 Open으로 되돌렸습니다. (이전 상태: <In Progress|Review>)
```
...
- **1단계**: 사용자 입력 수신 및 안내 (접두사: \033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m)
- **2단계**: 내부 모호성 분석 (사용자에게 비노출)
- **3단계**: 웹검색/코드탐색 자율 수행 (Step 1-4의 감지 신호 테이블 참조)
- **4단계**: G1~G4 게이트 평가 및 동적 선택지 출력 — Step 1-4와 동일한 구조(헤더, 간단 요약, 동적 선택지 `1.`~`5.` + `0.` 완료, 안내 문구)를 따르되 접두사는 \033[36m[T-NNN]\033[0m : \033[33m[WF -o]\033[0m를 사용한다
...
5. **AskUserQuestion 미사용**: 모든 사용자 입력은 텍스트 메뉴 출력 후 자유 입력으로 수신합니다. 접두사는 \033[36m[T-NNN]\033[0m : \033[33m[WF -플래그]\033[0m 형식을 사용합니다
