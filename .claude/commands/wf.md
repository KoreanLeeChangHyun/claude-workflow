...
```
T-NNN 티켓이 생성되었습니다. (파일: .claude.workflow/kanban/open/T-NNN.xml)
```
...
Glob 도구로 `.claude.workflow/kanban/open/T-NNN.xml`, `.claude.workflow/kanban/progress/T-NNN.xml`, `.claude.workflow/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. Read 도구로 XML의 `<metadata>/<status>`를 확인하여 분기합니다:
...
| `Done` (.claude.workflow/kanban/done/ 발견) | `flow-kanban move T-NNN open --force` 실행, 1-B-o-2로 진행 |
...
Glob 도구로 `.claude.workflow/kanban/open/T-NNN.xml`, `.claude.workflow/kanban/progress/T-NNN.xml`, `.claude.workflow/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다.
...
Glob 도구로 `.claude.workflow/kanban/open/T-NNN.xml`, `.claude.workflow/kanban/progress/T-NNN.xml`, `.claude.workflow/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. 미발견 시 에러 출력 후 종료.
...
Glob 도구로 `.claude.workflow/kanban/open/T-NNN.xml`, `.claude.workflow/kanban/progress/T-NNN.xml`, `.claude.workflow/kanban/review/T-NNN.xml` 패턴을 순서대로 탐색하고 Read 도구로 `<status>` 요소를 확인하여 분기합니다:
...
> `flow-kanban done`은 상태 갱신과 파일 이동(상태별 디렉터리(`open/`, `progress/`, `review/`) -> `.claude.workflow/kanban/done/T-NNN.xml`)을 내부적으로 처리합니다.
...
Glob 도구로 `.claude.workflow/kanban/open/T-NNN.xml`, `.claude.workflow/kanban/progress/T-NNN.xml`, `.claude.workflow/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. 미발견 시 `.claude.workflow/kanban/done/T-NNN.xml`도 확인합니다. 어디에서도 찾지 못한 경우 에러 출력 후 종료.
