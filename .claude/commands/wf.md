...
이전 대화 내역에 구체적인 작업 요청(구현 대상, 버그 내용, 리팩토링 목표, 조사 주제 등)이 포함되어 있는지 LLM이 자율적으로 판단한다. 키워드 매칭이 아닌 대화 흐름의 의미적 맥락을 기준으로 한다.
...
```
**`[WF -o]`** 이전 대화를 기반으로 다음과 같이 추론했습니다:
...
- `1.` 확인: 추론된 용도·goal·target·context를 Step 1-3 및 Step 1-4의 초기값으로 전달하여 진행합니다. Step 1-3에서는 추론된 용도에 해당하는 command 값을 사용하고, Step 1-4 루프 진입 시 goal/target/context가 이미 채워진 상태로 시작합니다
...
- **4단계**: G1~G4 게이트 평가 및 동적 선택지 출력 — Step 1-4와 동일한 구조(헤더, 간단 요약, 동적 선택지 `1.`~`5.` + `0.` 완료, 안내 문구)를 따르되 접두사는 **`[WF -o]`**를 사용한다

**최초 사이클 (`<current>` 값이 0인 경우)**:
- 아직 subnumber가 없으므로 새 워크플로우를 정의하는 대화를 진행합니다
- 사용자 "0. 완료" 선택 시 `flow-kanban add-subnumber` 호출하여 첫 번째 subnumber를 생성합니다:
  ```bash
  flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"
  ```
...
- 사용자 "0. 완료" 선택 시 `flow-kanban add-subnumber` 호출하여 다음 subnumber를 추가합니다:
  ```bash
  flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"
  ```
