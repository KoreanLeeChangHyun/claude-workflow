...
Glob 도구로 `.kanban/T-NNN.xml` 패턴을 검색합니다. Read 도구로 XML의 `<metadata>/<status>`를 확인하여 분기합니다:
...
- 존재하면: `mv ".kanban/done/T-NNN.xml" ".kanban/T-NNN.xml"` 실행 후 `flow-kanban move T-NNN open` 실행
...
Glob 도구로 `.kanban/T-NNN.xml` 패턴을 검색합니다. 미발견 시 에러 출력 후 종료.
...
> `flow-kanban done`은 상태 갱신과 파일 이동(`.kanban/T-NNN.xml` -> `.kanban/done/T-NNN.xml`)을 내부적으로 처리합니다.
...
Glob 도구로 `.kanban/T-NNN.xml` 패턴을 검색합니다. 미발견 시 `.kanban/done/T-NNN.xml`도 확인합니다. 어디에서도 찾지 못한 경우 에러 출력 후 종료.
...
> **판단 원칙**: 편집 의도가 명시적으로 없는 경우 `-o`(채번+용도선택만)를 선택합니다. 편집 의도가 있거나 불명확한 경우 `-e`(편집 루프 포함)를 선택하여 기존 동작과의 호환성을 유지합니다.
