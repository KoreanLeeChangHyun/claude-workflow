---
description: "티켓 라이프사이클 통합 관리. -o(생성), -p(수정), -s(제출), -d(종료), -c(삭제) 5개 플래그로 티켓 생성부터 완료까지 단일 진입점으로 제어합니다. Use when: 티켓 생성, 티켓 편집, 워크플로우 실행, 티켓 종료, 티켓 삭제를 한 번에 처리할 때"
argument-hint: "[-o|-p|-s|-d|-c] [N] (티켓 라이프사이클 통합 관리)"
---

# wf (Workflow 통합 명령어)

티켓 라이프사이클 전체를 단일 진입점으로 관리합니다. `-o`(Open), `-p`(Prompt), `-s`(Submit), `-d`(Done), `-c`(Cancel/삭제) 5개 플래그로 생성부터 종료까지 제어합니다.
...
1. **플래그 추출**: `$ARGUMENTS`에서 `-o`, `-p`, `-s`, `-d`, `-c` 패턴을 순서대로 검색합니다
...
| `/wf -p N` | 기존 티켓 프롬프트 편집 | `/wf -p 3` |
| `/wf -s N` | 티켓 제출 및 워크플로우 실행 | `/wf -s 3` |
| `/wf -d N` | 티켓 종료 (Done 상태로 이동) | `/wf -d 3` |
| `/wf -c N` | 티켓 삭제 (완전 삭제) | `/wf -c 3` |

현재 칸반 상태를 확인하려면 `.kanban/board.html`를 참조하세요.
```
...
| 조건 | 실행 흐름 |
|------|----------|
| `-o` 플래그 | Step 1 (Open/생성) |
| `-p` 플래그 | Step 2 (Prompt/수정) |
| `-s` 플래그 | Step 3 (Submit/제출) |
| `-d` 플래그 | Step 4 (Done/종료) |
| `-c` 플래그 | Step 5 (Cancel/삭제) |
| 플래그 없음 | 도움말 메뉴 출력 후 종료 |

---

## Step 1. `-o` 플래그: 티켓 생성 + 프롬프트 작성
...
Read 도구로 `.claude/scripts/data/prompt.txt`를 읽어 메뉴 항목을 로드한 뒤, 각 항목 앞에 번호를 붙여 출력합니다. "D. 완료"는 항상 선택지 맨 마지막에 배치합니다:

```
**`[WF -o]`** 어떤 목적의 티켓을 생성할까요?
...
사용자가 번호를 선택하면 용도가 결정됩니다. 사용자가 "D. 완료"를 선택하면 티켓 생성 없이 종료합니다.

### 1-3. 빈 티켓 즉시 생성

용도 선택 즉시 Bash 도구로 아래 명령을 실행하여 채번, board.html 갱신, 빈 제목의 티켓 XML 파일 생성을 처리합니다:

```bash
flow-kanban create "" --command <command값>
```

`flow-kanban create`는 board.html의 Open 컬럼에 새 항목을 추가하고 `.kanban/T-NNN.xml` XML 파일을 생성합니다. 생성된 XML에는 `<number>`, `<datetime>`, `<status>`, `<current>0</current>`, `<title>` 요소가 포함됩니다. `<current>`가 0이므로 subnumber는 아직 없는 상태입니다.
...
현재 프롬프트 상태를 반영한 안내와 개선 제안을 출력합니다 (접두사: **`[WF -o]`**). 사용자 입력을 기반으로 goal, target, constraints, criteria, context 정보를 개선합니다.
...
3. **동적 선택지**: 대화 맥락을 분석하여 `1.`~`5.` (최대 5개) + `D.` 완료를 생성. 백틱은 선택지 번호(`1.`~`5.`)와 `D.`에만 사용하고 나머지 문구에는 백틱을 사용하지 않는다. `D.` 완료는 항상 선택지 맨 마지막에 배치한다
...
> `<command>` 값은 1-3에서 설정된 값을 그대로 사용합니다. `flow-kanban add-subnumber`의 `--command` 인자에 동일 값을 전달합니다. constraints, criteria, context 값이 없는 경우 해당 인자를 생략합니다.
...
| 편집 | `/wf -p N` | 티켓 내용을 추가로 수정합니다 |
| 종료 | `/wf -d N` | 티켓을 Done 상태로 종료합니다 |
```

---

## Step 2. `-p` 플래그: 티켓 프롬프트 편집

> **칸반 전이**: Review/In Progress → **Open** (자동 복귀) → 편집 완료 후 Open 유지
...
```
-p 플래그는 티켓 번호(N)를 반드시 지정해야 합니다. 예: /wf -p 3
```
...
### 2-3. Review/In Progress 상태 자동 Open 복귀
...
```
T-NNN 티켓을 Open으로 되돌렸습니다. (이전 상태: <In Progress|Review>)
```
...
- **1단계**: 사용자 입력 수신 및 안내 (접두사: **`[WF -p]`**)
- **2단계**: 내부 모호성 분석 (사용자에게 비노출)
- **3단계**: 웹검색/코드탐색 자율 수행 (Step 1-4의 감지 신호 테이블 참조)
- **4단계**: G1~G4 게이트 평가 및 동적 선택지 출력 — Step 1-4와 동일한 구조(헤더, 간단 요약, 동적 선택지 `1.`~`5.` + `D.` 완료, 안내 문구)를 따르되 접두사는 **`[WF -p]`**를 사용한다

**최초 사이클 (`<current>` 값이 0인 경우)**:
- 아직 subnumber가 없으므로 새 워크플로우를 정의하는 대화를 진행합니다
- 사용자 "D. 완료" 선택 시 `flow-kanban add-subnumber` 호출하여 첫 번째 subnumber를 생성합니다:
  ```bash
  flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"
  ```
...
- 사용자 "D. 완료" 선택 시 `flow-kanban add-subnumber` 호출하여 다음 subnumber를 추가합니다:
  ```bash
  flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"
  ```

**`<command>` 태그 갱신 정책**: `-p` 플래그에서는 `<command>` 갱신이 허용됩니다. 사용자가 명시적으로 용도를 변경하려는 경우 `flow-kanban add-subnumber`의 `--command` 인자에 새 값을 전달합니다.

> `constraints`, `criteria`, `context` 값이 없는 경우 해당 인자를 생략합니다.
...
로드된 티켓 XML 파일을 XML 파서로 읽어 `<current>` 요소 값을 먼저 확인합니다.

- `<current>` 값이 `0` 또는 존재하지 않는 경우 (subnumber 없음): 에러 출력 후 종료
  ```
  T-NNN 티켓에 워크플로우가 정의되지 않았습니다. /wf -p N으로 먼저 작성하세요.
  ```

`<current>` 값이 1 이상인 경우, 해당 ID의 `<subnumber id="N">` 요소를 찾아 내부의 `<command>` 자식 요소를 읽습니다.

- 유효한 값: `implement`, `research`, `review`
- `<command>` 요소가 없는 경우:
  ```
  T-NNN 티켓에 <command> 태그가 없습니다.
  /wf -p N 으로 티켓 용도를 먼저 지정하세요.
  ```
- `<command>` 값이 유효하지 않은 경우:
  ```
  T-NNN 티켓의 <command>XXX</command> 값이 유효하지 않습니다. (허용: implement, research, review)
  /wf -p N 으로 티켓 용도를 다시 지정하세요.
  ```
...
### 3-5. 워크플로우 스킬 로드 및 실행

Read 도구로 `<command>` 값에 해당하는 wf 스킬을 직접 로드한 뒤 해당 스킬의 지침을 따라 워크플로우를 실행합니다.
...
> **참고**: 워크플로우 완료 후 `flow-kanban move T-NNN review`로 자동 전이됩니다 (`workflow-wf-submit` 스킬 지침에서 처리). 다음 사이클을 실행하려면 `/wf -p N`으로 프롬프트를 수정하거나 `/wf -d N`으로 티켓을 종료하세요.
...
- **exit code 1, "찾을 수 없습니다" 메시지**: 에러 출력 후 종료
  ```
  T-NNN 티켓을 board.html에서 찾을 수 없습니다.
  ```
- **exit code 1, "이미 Done" 메시지**: 안내 출력 후 종료
  ```
  T-NNN은 이미 Done 상태입니다.
  ```
- **exit code 0**: board.html 갱신과 파일 이동이 완료된 것이므로 4-3으로 진행합니다

> `flow-kanban done`은 board.html 갱신과 티켓 파일 이동(`.kanban/T-NNN.xml` → `.kanban/done/T-NNN.xml`)을 내부적으로 처리합니다. 별도의 Write 또는 mv 명령이 필요하지 않습니다.
...
지정한 티켓을 board.html에서 제거하고 XML 파일을 완전히 삭제합니다. `-d`(Done)와 달리 히스토리를 보존하지 않습니다.
...
```bash
flow-kanban delete T-NNN
```

`flow-kanban delete`는 board.html에서 해당 티켓 항목을 제거하고 XML 파일을 삭제합니다.
...
|--------|------------|------------|---------|
| `-o` | (없음) | Open | `flow-kanban create` |
| `-p` | Review/Progress | Open (자동 복귀) | `flow-kanban move T-NNN open` |
| `-p` | Open | Open (유지) | — |
| `-s` | Open | In Progress | workflow-wf-* 스킬 로드 후 실행 |
| `-s` (완료 후) | In Progress | Review | `flow-kanban move T-NNN review` (workflow-wf-submit 처리) |
| `-d` | Any | Done | `flow-kanban done T-NNN` |
| `-c` | Any | (삭제) | `flow-kanban delete T-NNN` |
...
|------|--------|---------------|---------|
| 티켓 생성 직후 | `-o` | `<current>0</current>` (subnumber 없음) | `flow-kanban create` |
| 워크플로우 정의 | `-o` 완료 또는 `-p` | subnumber N 추가, `<current>N</current>` 갱신 | `flow-kanban add-subnumber T-NNN --command ... --goal ... --target ...` |
| 워크플로우 실행 완료 | `-s` 후처리 | subnumber N에 workdir/result 기록 | `flow-kanban update-subnumber T-NNN --id N --workdir ... --result ...` |
| 다음 사이클 시작 | `-p` (추가 사이클) | subnumber N+1 추가, `<current>N+1</current>` 갱신 | `flow-kanban add-subnumber T-NNN --command ... --goal ... --target ...` |
| 티켓 종료 | `-d` | 변경 없음 (히스토리 보존) | `flow-kanban done T-NNN` |

## 주의사항

1. **단일 진입점**: 티켓 라이프사이클 전체를 `/wf` 하나로 관리합니다. `/wf`가 유일한 티켓 관리 진입점입니다
2. **자동 Open 복귀**: `-p` 플래그로 Review/In Progress 티켓에 접근 시 사용자 확인 없이 Open으로 자동 복귀합니다
3. **`<command>` 태그 정책**: `-o` 생성 시 설정된 값을 기본 보존하되, `-p` 편집 시 명시적 변경이 허용됩니다
4. **Bash 도구 사용**: 칸반 상태 전이 및 파일 이동이 필요한 Step(1-3, 2-3, 3-5, 4-2)에서 허용합니다
