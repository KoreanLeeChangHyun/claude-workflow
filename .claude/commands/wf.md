...
현재 칸반 상태를 확인하려면 `.claude-organic/kanban/` 디렉터리의 XML 티켓 파일을 참조하세요.
```
...
**트랙 B: 맥락 미감지 또는 fallback** -- Read 도구로 `.claude-organic/prompt/prompt.txt`를 읽어 메뉴 항목을 로드한 뒤 출력합니다:
...
```
T-NNN 티켓이 생성되었습니다. (파일: .claude-organic/kanban/open/T-NNN.xml)
```
...
Glob 도구로 `.claude-organic/kanban/open/T-NNN.xml`, `.claude-organic/kanban/progress/T-NNN.xml`, `.claude-organic/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. Read 도구로 XML의 `<metadata>/<status>`를 확인하여 분기합니다:
...
| `Done` (.claude-organic/kanban/done/ 발견) | `flow-kanban move T-NNN open --force` 실행, 1-B-o-2로 진행 |
...
**트랙 B: 맥락 미감지 또는 fallback** -- Read 도구로 `.claude-organic/prompt/prompt.txt`를 읽어 메뉴 항목을 로드합니다.
...
Glob 도구로 `.claude-organic/kanban/open/T-NNN.xml`, `.claude-organic/kanban/progress/T-NNN.xml`, `.claude-organic/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다.

**파일 발견 시**: XML `<status>` 요소에서 현재 칸반 상태를 판별합니다.

**파일 미발견 시**: `.claude-organic/kanban/done/T-NNN.xml`을 확인합니다:
...
Glob 도구로 `.claude-organic/kanban/open/T-NNN.xml`, `.claude-organic/kanban/progress/T-NNN.xml`, `.claude-organic/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. 미발견 시 에러 출력 후 종료.
...
Glob 도구로 `.claude-organic/kanban/open/T-NNN.xml`, `.claude-organic/kanban/progress/T-NNN.xml`, `.claude-organic/kanban/review/T-NNN.xml` 패턴을 순서대로 탐색하고 Read 도구로 `<status>` 요소를 확인하여 분기합니다:
...
`.claude-organic/workflow/` 하위에서 해당 티켓의 가장 최근 워크플로우 디렉터리를 탐색합니다:
- Glob 도구로 `.claude-organic/workflow/*/T-NNN*/*/report.md` 또는 `.claude-organic/workflow/*/*/implement/report.md` 등을 검색합니다
- 티켓 XML의 `<result>` 요소의 `<workdir>` 값을 확인하여 정확한 워크플로우 디렉터리를 특정합니다
- `<workdir>` 값이 없으면 `.claude-organic/workflow/` 하위에서 최신 타임스탬프 디렉터리를 탐색합니다
...
성공 시 종료 메시지:
```
`[T-NNN]` : `[WF -d]` T-NNN 티켓이 Done 상태로 종료되었습니다. (파일: .claude-organic/kanban/done/T-NNN.xml)
```
...
> `flow-kanban done`은 상태 갱신과 파일 이동(상태별 디렉터리(`open/`, `progress/`, `review/`) -> `.claude-organic/kanban/done/T-NNN.xml`)을 내부적으로 처리합니다.

종료 메시지:
```
T-NNN 티켓이 Done 상태로 종료되었습니다. (파일: .claude-organic/kanban/done/T-NNN.xml)
```
...
Glob 도구로 `.claude-organic/kanban/open/T-NNN.xml`, `.claude-organic/kanban/progress/T-NNN.xml`, `.claude-organic/kanban/review/T-NNN.xml` 패턴을 순서대로 검색합니다. 미발견 시 `.claude-organic/kanban/done/T-NNN.xml`도 확인합니다. 어디에서도 찾지 못한 경우 에러 출력 후 종료.
