...
현재 칸반 상태를 확인하려면 `.claude.workflow/kanban/` 디렉터리의 XML 티켓 파일을 참조하세요.
```
...
- `1.` 확인: 추론된 용도에 해당하는 command 값으로 `flow-kanban add-subnumber T-NNN --command <command값> --goal "<추론된 goal>" --target "<추론된 target>" --skip-validation` 호출 후 1-3으로 진행 (`-o` 모드는 편집 루프 미진입이므로 품질 검증을 건너뜁니다)
- `2.` 용도 직접 지정: 트랙 B로 전환
- `0.` 취소: 티켓 생성 없이 종료

**트랙 B: 맥락 미감지 또는 fallback** -- Read 도구로 `.claude.workflow/prompt/prompt.txt`를 읽어 메뉴 항목을 로드한 뒤 출력합니다:
...
사용자가 번호를 선택하면 해당 command 값으로 `flow-kanban add-subnumber T-NNN --command <command값> --goal "(미정)" --target "(미정)" --skip-validation`을 호출합니다 (`--goal`/`--target`은 CLI 필수 인자이므로 placeholder를 전달하고, `-o` 모드는 편집 루프 미진입이므로 `--skip-validation`으로 품질 검증을 건너뜁니다). "0. 완료" 선택 시 종료합니다.

#### 1-3. 완료 메시지 출력

```
T-NNN 티켓이 생성되었습니다. (파일: .claude.workflow/kanban/active/T-NNN.xml)
```
...
Glob 도구로 `.claude.workflow/kanban/active/T-NNN.xml` 패턴을 검색합니다. Read 도구로 XML의 `<metadata>/<status>`를 확인하여 분기합니다:
...
| `Done` (.claude.workflow/kanban/done/ 발견) | `.claude.workflow/kanban/done/T-NNN.xml`을 `.claude.workflow/kanban/active/T-NNN.xml`로 복원한 뒤 `flow-kanban move T-NNN open --force` 실행, 1-B-o-2로 진행 |
...
편집 중 상태를 활성화합니다:
```bash
flow-kanban set-editing T-NNN --on
```
...
- `2.` 용도 직접 지정: 트랙 B로 전환
- `0.` 취소: `flow-kanban set-editing T-NNN --off` 실행 후 종료

**트랙 B: 맥락 미감지 또는 fallback** -- Read 도구로 `.claude.workflow/prompt/prompt.txt`를 읽어 메뉴 항목을 로드합니다.

**용도->command 매핑**: 연구=`research`, 구현/버그수정/리팩토링/아키텍처설계=`implement`, 리뷰=`review`

"0. 완료" 선택 시 `flow-kanban set-editing T-NNN --off` 실행 후 종료합니다.
...
- 사용자 "0. 완료" 선택 시 아래 순서로 처리:
  ...
  2. `flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"` (context 없으면 생략)
  3. Read 도구로 갱신된 XML 파일을 읽어 subnumber 내용 확인 후 Step 1-5로 진행

`flow-kanban set-editing T-NNN --off` 실행하여 편집 중 상태를 해제합니다.
...
Glob 도구로 `.claude.workflow/kanban/active/T-NNN.xml` 패턴을 검색합니다.

**파일 발견 시**: XML `<status>` 요소에서 현재 칸반 상태를 판별합니다.

**파일 미발견 시**: `.claude.workflow/kanban/done/T-NNN.xml`을 확인합니다:
- 존재하면: `mv ".claude.workflow/kanban/done/T-NNN.xml" ".claude.workflow/kanban/active/T-NNN.xml"` 실행 후 `flow-kanban move T-NNN open` 실행
- 어디에서도 찾지 못한 경우: 에러 출력 후 종료 (`T-NNN 티켓 파일을 찾을 수 없습니다.`)

티켓 파일 로드 완료 후 편집 중 상태를 활성화합니다:
```bash
flow-kanban set-editing T-NNN --on
```
...
- "0. 완료" 선택 시 constraints/criteria 필수 검증(10자 이상) 후 `flow-kanban add-subnumber` 호출하여 첫 번째 subnumber를 생성합니다

**추가 사이클 (`<current>` 값이 1 이상인 경우)**:
- 현재 `<subnumber id="N">` 내용을 표시하여 이전 작업 맥락을 제공합니다
- `<submit>` 내 `active="true"` subnumber의 `<result>` 요소 유무를 확인합니다

**분기 A -- result 없음 (미제출): 기존 subnumber 수정**
- "0. 완료" 선택 시 constraints/criteria 필수 검증 후 `flow-kanban update-subnumber T-NNN --id <활성subnumber_id> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"`

**분기 B -- result 있음 (제출 완료): 새 subnumber 추가**
- "0. 완료" 선택 시 constraints/criteria 필수 검증 후 `flow-kanban add-subnumber T-NNN --command <command값> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"`

subnumber 추가/갱신 완료 후 `flow-kanban set-editing T-NNN --off` 실행하여 편집 중 상태를 해제합니다.

> **constraints/criteria 필수 검증**: 모든 사이클에서 "0. 완료" 선택 시 constraints 또는 criteria가 누락이거나 10자 미만이면 메시지를 출력하고 루프를 계속합니다.

> **`<command>` 태그 갱신 정책**: `-e NNN` 편집 시 `<command>` 갱신이 허용됩니다. `update-subnumber`는 `--command` 인자를 지원하지 않으므로, command 변경이 필요한 경우 add-subnumber 분기(제출 후 편집)를 안내합니다.
...
Glob 도구로 `.claude.workflow/kanban/active/T-NNN.xml` 패턴을 검색합니다. 미발견 시 에러 출력 후 종료.

#### 2-3. `<command>` 태그 파싱

XML에서 `<current>` 값을 확인합니다:
- `0` 또는 미존재: 에러 출력 후 종료 (`워크플로우가 정의되지 않았습니다. /wf -e N으로 먼저 작성하세요.`)
- 1 이상: 해당 ID의 `<subnumber>` 직하 `<command>` 자식 요소를 읽습니다
...
> 워크플로우 완료 후 `flow-kanban move T-NNN review`로 자동 전이됩니다. 체인 command인 경우 finalization.py가 완료 후 자동으로 다음 스테이지를 새 tmux 세션에서 실행합니다.
...
Glob 도구로 `.claude.workflow/kanban/active/T-NNN.xml`을 탐색하고 Read 도구로 `<status>` 요소를 확인하여 분기합니다:
...
`.claude.workflow/workflow/` 하위에서 해당 티켓의 가장 최근 워크플로우 디렉터리를 탐색합니다:
- Glob 도구로 `.claude.workflow/workflow/*/T-NNN*/*/report.md` 또는 `.claude.workflow/workflow/*/*/implement/report.md` 등을 검색합니다
- 티켓 XML의 활성 subnumber에서 `<result>` 요소의 `<workdir>` 값을 확인하여 정확한 워크플로우 디렉터리를 특정합니다
- `<workdir>` 값이 없으면 `.claude.workflow/workflow/` 하위에서 최신 타임스탬프 디렉터리를 탐색합니다
...
성공 시 종료 메시지:
```
`[T-NNN]` : `[WF -d]` T-NNN 티켓이 Done 상태로 종료되었습니다. (파일: .claude.workflow/kanban/done/T-NNN.xml)
```

**"2. 상세 review" 선택 시:**
- 기존 티켓의 `<command>` 값을 확인합니다
- `flow-kanban add-subnumber T-NNN --command review --goal "T-NNN 구현 결과 상세 리뷰" --target "<보고서 경로>" --constraints "간단 검토에서 발견된 경고 항목을 중점 검토" --criteria "모든 WARN 항목이 해소되거나 수용 근거가 명시됨" --context "간단 검토에서 경고 발견" --skip-validation` 를 실행하여 새 review subnumber를 추가합니다
- 안내 메시지를 출력합니다:
```
`[T-NNN]` : `[WF -d]` 상세 review 워크플로우를 제출합니다.

| 목적 | 커맨드 | 설명 |
|------|--------|------|
| 제출 | `/wf -s N` | review 워크플로우 실행 <- 권장 |
| 편집 | `/wf -e N` | review 프롬프트를 먼저 편집합니다 |
```
...
> `flow-kanban done`은 상태 갱신과 파일 이동(`.claude.workflow/kanban/active/T-NNN.xml` -> `.claude.workflow/kanban/done/T-NNN.xml`)을 내부적으로 처리합니다.

종료 메시지:
```
T-NNN 티켓이 Done 상태로 종료되었습니다. (파일: .claude.workflow/kanban/done/T-NNN.xml)
```
...
Glob 도구로 `.claude.workflow/kanban/active/T-NNN.xml` 패턴을 검색합니다. 미발견 시 `.claude.workflow/kanban/done/T-NNN.xml`도 확인합니다. 어디에서도 찾지 못한 경우 에러 출력 후 종료.
...
|--------|------------|------------|---------|
| `-o` (번호 없음) | (없음) | Open | `flow-kanban create "" --command init` + `flow-kanban add-subnumber --skip-validation` (채번+용도선택만, 편집 루프 미진입) |
| `-e` (번호 없음) | (없음) | Open | `flow-kanban create "" --command init` + `flow-kanban add-subnumber` + 편집 루프 (`-oe`도 동일 동작) |
| `-o N` | Done | Open (복원) | `mv .claude.workflow/kanban/done/T-NNN.xml .claude.workflow/kanban/active/T-NNN.xml` + `flow-kanban move T-NNN open` (내용 표시만, 편집 루프 미진입) |
| `-o N` | Review/In Progress | Open (즉시 전환) | `flow-kanban move T-NNN open --force` (내용 표시만, 편집 루프 미진입) |
| `-o N` | Open | Open (유지) | -- (내용 표시만, 편집 루프 미진입) |
| `-e N` | Done | Open (복원) | `mv .claude.workflow/kanban/done/T-NNN.xml .claude.workflow/kanban/active/T-NNN.xml` + `flow-kanban move T-NNN open` + 편집 루프 (`-oe N`도 동일 동작) |
...
| `-d` | Review | Review (상세 review 선택 시) | `flow-kanban add-subnumber T-NNN --command review` |
...
| 워크플로우 정의 | `-o` (채번+용도선택만) | subnumber N 추가(기본값), `<current>N</current>` 갱신 | `flow-kanban add-subnumber T-NNN --command ... --goal "..." --target "..." --skip-validation` (품질 검증 건너뜀) |
...
11. **품질 검증 (prompt_validator)**: `flow-kanban add-subnumber` 호출 시 `prompt_validator.py`가 자동으로 품질 점수를 계산합니다. 검증 대상 태그는 `goal`, `target`, `constraints`, `criteria` 4개이며 공식은 `score = (존재_태그수/4) × 0.6 + (유효_태그수/4) × 0.4`입니다 (유효 = 10자 이상 & `TODO:` 미시작). 임계값은 `QUALITY_THRESHOLD = 0.6`이며 미달 시 subnumber가 자동 롤백되고 exit code 1로 종료됩니다. `--skip-validation` 플래그를 추가하면 품질 검증을 건너뜁니다. `-o` 모드(채번+용도선택만)와 `-d` 상세 review 생성 시에는 편집 루프를 거치지 않으므로 `--skip-validation`을 사용합니다. `-e` 모드의 편집 루프에서는 constraints/criteria 10자 이상 검증이 선행되므로 품질 검증을 통과할 수 있습니다
