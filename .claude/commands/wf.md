...
> 참고: 여러 워크플로우를 순차 실행하려면 `<command>` 태그에 `research>implement>review` 형식으로 체인을 지정할 수 있습니다. 선택된 상태가 `todo`이면 `/wf -s N` 실행 전에 `/wf -e N`으로 Open 승격이 필요합니다.
```
...
(선택적으로, 사용자 동의 하에 `flow-kanban move T-NNN open --force`로 자동 승격 후 진행할 수 있으나, 기본 동작은 에러 종료입니다.)
...
> **체인 command 전달 규칙**: 오케스트레이터의 INIT 1-Step (`cd "$(flow-init <command> --ticket T-NNN | tail -1)"`) 호출 시, 체인인 경우 전체 문자열(예: `research>implement`)을 `<command>` 인자로 그대로 전달합니다. initialization.py가 `>` 구분자를 감지하여 첫 세그먼트를 실제 실행 command로, 전체 문자열을 `.context.json`의 `command` 필드에 기록합니다.
...
| "이거 박제해줘", "나중에 할 거야", "언젠가 할 일", "백로그에 넣어" | `-o` 또는 `-e` | To Do | 미래 작업 의도 명시 |
...
> (4) **상태 선택**: "박제/나중에/언젠가/백로그" -> To Do 추천, "지금/바로/이번에/집중" -> Open 추천, 무맥락이면 번호 메뉴로 질의
