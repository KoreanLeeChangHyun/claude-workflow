...
전환 대상이 `main` 또는 `master`인 경우, `git checkout <브랜치명>` 완료 후 자동으로 pull을 실행합니다:
...
- `-b <이름>`: `origin/main` 기반으로 새 브랜치 생성 (자동 fetch 후)
...
| 조건 | 실행 명령 |
|------|----------|
| 베이스 미지정 | `git checkout -b <이름> origin/main` |
| 베이스 지정 | `git checkout -b <이름> <베이스>` |
...
| **pull 충돌** | "자동 pull 중 충돌이 발생했습니다. `git status`로 충돌 파일을 확인하고 해결하세요." 안내 |
