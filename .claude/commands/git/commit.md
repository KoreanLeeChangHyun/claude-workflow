...
커밋 시 `.claude-organic/.version` (SemVer: `MAJOR.MINOR.PATCH`) 파일의 버전 갱신 여부를 판단합니다.
...
2. `.claude/` 변경이 있으면 `.claude-organic/.version` 의 PATCH를 +1 증가
3. 증가된 `.version` 파일을 커밋 대상에 포함
4. **커밋 계획**에 버전 갱신 여부를 표시: `**버전**: 1.0.0 → 1.0.1`

> `.claude-organic/.version` 자체의 변경만 있는 경우(이미 수동 갱신됨)는 추가 증가하지 않습니다.
