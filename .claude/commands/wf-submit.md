...
> **칸반 전이**: Open → **Submit** → **In Progress** (Submit 전환은 wf-submit.md 즉시 수행, In Progress 전환은 flow-init 실행 시 처리)
...
Bash 도구로 아래 명령을 실행하여 티켓 상태를 즉시 Submit으로 전환합니다:

```bash
flow-kanban move T-NNN submit
```

T-NNN은 `$ARGUMENTS`에서 파싱한 티켓 번호입니다 (2-1 섹션 참조).

**실패 처리**: `flow-kanban move` 실패(exit code 1) 시 아래 경고 메시지를 출력하되 워크플로우 실행은 계속 진행합니다 (비차단):

```
경고: T-NNN 칸반 상태를 Submit으로 전환하지 못했습니다. 워크플로우는 계속 실행됩니다.
```
...
> **체인 command 전달 규칙**: 스킬 파일 로드 후 오케스트레이터가 step-init의 `flow-init <command>` 호출 시, **체인인 경우 전체 문자열**(예: `research>implement`)을 `<command>` 인자로 전달해야 합니다. initialization.py가 `>` 구분자를 감지하여 첫 세그먼트를 실제 실행 command로, 전체 문자열을 `.context.json`의 `command` 필드에 기록합니다. 이 전체 문자열이 없으면 finalization.py가 체인 후속 스테이지를 발사할 수 없습니다.

> **참고**: 워크플로우 실행 전 티켓은 Submit 상태로 전환되며, flow-init 실행 시 In Progress로 전환됩니다. 워크플로우 완료 후 `flow-kanban move T-NNN review`로 자동 전이됩니다 (`workflow-wf-submit` 스킬 지침에서 처리). 다음 사이클을 실행하려면 `/wf -e N`으로 프롬프트를 수정하거나 `/wf -d N`으로 티켓을 종료하세요. 체인 command인 경우 finalization.py가 완료 후 자동으로 다음 스테이지를 새 tmux 세션에서 실행합니다.
