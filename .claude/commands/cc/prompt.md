...
argument-hint: "[-p|-q|-d] [#N] (옵션 없으면 선택지 제공, #N으로 티켓 번호 지정)"
skills:
...
1. 티켓 작성 — 티켓(T-NNN.txt)을 대화로 작성/개선합니다
...
3. `-p` → Step 1로 진행 (티켓 공동 작성 흐름)
4. `-q` → Step Q로 분기. 플래그 뒤에 텍스트가 있으면 즉시 작업 요청으로 사용
5. `-d` → Step D로 분기. 반드시 #N 티켓 번호를 함께 지정해야 하며, 미지정 시 에러 메시지 출력 후 종료
...
`$ARGUMENTS`에서 `#N` 패턴(예: `#3`, `#12`)을 파싱하여 티켓 번호를 추출하고 3자리 zero-padding을 적용합니다 (예: `#3` → `T-003`).

`#N` 패턴이 없으면 아래 에러 메시지를 출력하고 종료합니다:
...
(a) Read 도구로 `.kanban/quick.txt`를 읽습니다. 파일이 없으면 빈 상태로 시작합니다.

(b) Glob 도구로 `.uploads/**/*` 패턴을 검색하여 첨부 파일 존재를 확인하고, 파일이 존재하면 Read 도구로 각 파일을 읽어 컨텍스트에 추가합니다.

> **매 대화 시작 시 필수 읽기**: `.kanban/quick.txt`와 `.uploads/**/*` 두 항목 모두 Quick 모드 진입 시 반드시 읽어야 합니다. 파일이 없거나 비어있어도 확인 단계는 건너뛰지 않습니다.
...
매 턴 종료 시 `.kanban/quick.txt`를 Write 도구로 갱신합니다. 형식은 아래 3섹션으로 고정합니다:
...
`.kanban/quick.txt`를 빈 파일로 초기화(Write 도구로 빈 문자열 저장)한 뒤 완료 메시지를 출력하고 종료합니다. 워크플로우 연결 없이 종료합니다 (FSM 상태 전이, 배너 출력 없음).
...
Read 도구로 `.kanban/board.md`를 읽어 기존 티켓 목록을 파악합니다.
...
- 새 티켓 생성 또는 기존 티켓 선택을 Step 2에서 결정합니다.

**파일 경로 자동 인식**: 티켓 파일 내용이 파일 경로 패턴(`.workflow/`, `src/`, `.claude/` 등으로 시작하거나 `.md`, `.py`, `.ts`, `.json` 등 확장자로 끝나는 단일 행)인 경우, 해당 파일을 Read 도구로 자동으로 읽어 **참조 컨텍스트**로 활용합니다.

- 파일 경로 행은 티켓 파일의 원본 내용으로 보존합니다 (덮어쓰지 않음)
- 읽어들인 파일 내용은 Step 2에서 현재 상태를 표시할 때 함께 요약하고, Step 3 대화 루프에서 티켓 구체화의 기반 컨텍스트로 사용합니다
...
티켓 파일 내용이나 `.uploads/` 컨텍스트에서 **용도를 판별**하여 추가 references를 선택적으로 로드합니다:
...
> **"Q" 또는 "Quick 모드로 전환"** 입력 시 → 현재 티켓 파일 내용을 보존한 채 Step Q로 분기합니다.
...
#### 기존 티켓 편집 (티켓 번호 지정됨 또는 기존 티켓 선택)
...
사용자와 자유롭게 대화하면서 티켓 파일(T-NNN.txt)을 점진적으로 개선합니다.
...
```
**`[PROMPT]`** <현재 prompt 상태를 반영한 자연스러운 안내 문구 + 구체적 제안>
...
>
> **"Q" 또는 "Quick 모드로 전환"** 입력 시 → 현재 티켓 파일 내용을 보존한 채 Step Q로 분기합니다. Quick 모드에서 조사/작업 후 다시 "티켓 작성으로 전환"을 선택하면 Step 1로 돌아올 수 있습니다.
...
**탐색/검색 결과의 `<context>` 태그 구조적 기록**: 코드베이스 탐색이나 웹검색을 수행한 경우, 탐색 결과의 핵심 정보를 티켓 파일(T-NNN.txt)의 `<context>` 태그 내에 항목별로 구조화하여 기록합니다. `<context>` 태그가 아직 없으면 새로 추가하고, 이미 있으면 내용을 보강합니다.
...
내부 모호성 분석 결과와 함께, 아래 **G1~G4 게이트 조건**을 티켓 파일 현재 내용에 대해 내부적으로 평가합니다 (스크립트 호출 불필요, LLM 내부 판단):
...
> **XML 태그 미사용 폴백**: 티켓 파일에 XML 태그가 전혀 없는 경우, G1~G4 대신 기존 자연어 기준(목표 명확성, 대상 특정성, 제약 조건 포함, 검증 기준 존재)으로 동일하게 평가합니다.
...
```
**`[PROMPT]`** 현재 prompt가 충분히 구체적으로 작성된 것 같습니다.
...
- **"Q"** 선택 시 현재 티켓 파일 내용을 보존한 채 Step Q로 분기합니다.
- 그 외 자유 텍스트 입력 시 Step 3으로 돌아갑니다.

- **1~3개 통과** → 완료를 제안하지 않고, 미충족 게이트에 해당하는 개선 방향을 텍스트 메뉴로 제시합니다:

```
**`[PROMPT]`** prompt를 더 구체화하면 planner가 더 정확한 계획을 수립할 수 있습니다. 어떤 부분을 보강할까요?
...
최종 티켓 파일 내용을 표시하고 변경 요약을 출력합니다:
...
티켓 작성이 완료된 후, 작성된 티켓 파일의 내용에 따라 자연스럽게 다음 커맨드로 전환합니다. 티켓에 담긴 요구사항의 성격에 맞춰 적절한 커맨드를 선택하여 `#N` 티켓 번호와 함께 실행하면, 해당 커맨드가 티켓 내용을 자동으로 참조하고 작업을 진행합니다.

**선택 기준:**

- **구현 목적**: 새 기능, 코드 수정, 모듈 개발, 아키텍처 설계 등 → `/cc:implement #N` 사용
