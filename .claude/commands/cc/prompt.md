...
1. `$ARGUMENTS`가 비어있거나 옵션 플래그(`-p`, `-q`)가 없는 경우 → AskUserQuestion으로 모드 선택지를 제시:
...
AskUserQuestion(
  questions: [{
    ...
    options: [
      ...
      { label: "-q Quick 모드", description: "워크플로우 없이 질문/조사/수정을 자유롭게 처리" }
    ],
    multiSelect: false
  }]
)
...
사용자 요청(`$ARGUMENTS`에서 `-q` 뒤 텍스트 또는 AskUserQuestion으로 수신)에 대해 바닐라 클로드 코드로 자유롭게 처리합니다. 질문/조사/수정 모든 작업이 가능합니다.

- 텍스트가 없으면 AskUserQuestion으로 작업 요청을 수신합니다
- 코드베이스 탐색: Glob, Grep, Read 사용
- 웹 조사: WebSearch, WebFetch 사용
- 파일 수정: Read 후 Edit 또는 Write 사용

**사용 가능 도구**: Read, Write, Edit, Glob, Grep, WebSearch, WebFetch, AskUserQuestion
...
AskUserQuestion(
  questions: [{
    question: "추가 작업이 있으신가요?",
    header: "Quick 모드",
    options: [
      { label: "추가 작업", description: "다른 작업을 계속합니다" },
      { label: "종료", description: "Quick 모드를 마칩니다" }
    ],
    multiSelect: false
  }]
)
...
완료 메시지를 출력한 후 종료합니다. 워크플로우 연결 없이 종료합니다 (FSM 상태 전이, 배너 출력 없음).
...
6. **Quick 모드 범위 제한**: Quick 모드는 워크플로우(FSM/가드/서브에이전트)를 사용하지 않습니다. 복잡한 작업은 cc:implement, cc:research, cc:review를 사용하세요
