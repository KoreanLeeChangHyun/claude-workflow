...
Bash 도구로 아래 명령을 실행하여 편집 중 상태를 활성화합니다:
...
- `0.` 취소: Bash 도구로 `flow-kanban set-editing T-NNN --off`를 실행하여 편집 플래그를 해제한 후 티켓 생성 없이 종료합니다
...
사용자가 번호를 선택하면 용도가 결정됩니다. 사용자가 "0. 완료"를 선택하면 Bash 도구로 `flow-kanban set-editing T-NNN --off`를 실행하여 편집 플래그를 해제한 후 티켓 생성 없이 종료합니다.
...
Bash 도구로 아래 명령을 실행하여 편집 중 상태를 해제합니다:

```bash
flow-kanban set-editing T-NNN --off
```
...
티켓 파일 로드 완료 후 Bash 도구로 아래 명령을 실행하여 편집 중 상태를 활성화합니다:
...
- goal, target, constraints, criteria, context를 수집하는 대화를 진행합니다

**분기 A — result 없음 (미제출): 기존 subnumber 수정**
- 활성 subnumber에 `<result>`가 없는 경우(아직 제출되지 않은 상태)에 해당합니다
- 사용자 "0. 완료" 선택 시 constraints/criteria 필수 검증 후 `flow-kanban update-subnumber` 호출하여 기존 subnumber를 수정합니다:
  - **constraints/criteria 필수 검증**: constraints 또는 criteria가 누락이거나 10자 미만이면 아래 메시지를 출력하고 루프를 계속합니다 (update-subnumber를 실행하지 않음):
    ```
    `[T-NNN]` : `[WF -e]` constraints 또는 criteria가 누락되었거나 내용이 부족합니다(10자 이상 필요). 보강 후 다시 완료를 선택하세요.
    ```
  - 검증 통과 시:
    ```bash
    flow-kanban update-subnumber T-NNN --id <활성subnumber_id> --goal "<goal>" --target "<target>" --constraints "<constraints>" --criteria "<criteria>" --context "<context>"
    ```

**분기 B — result 있음 (제출 완료): 새 subnumber 추가**
- 활성 subnumber에 `<result>`가 존재하는 경우(이전 사이클이 완료된 후 새 사이클을 시작하는 상황)에 해당합니다
...
Bash 도구로 아래 명령을 실행하여 편집 중 상태를 해제합니다 (subnumber 추가/갱신 완료 후):

```bash
flow-kanban set-editing T-NNN --off
```

**`<command>` 태그 갱신 정책**: `-e NNN` 편집 시 `<command>` 갱신이 허용됩니다. 사용자가 명시적으로 용도를 변경하려는 경우 `flow-kanban add-subnumber`의 `--command` 인자에 새 값을 전달합니다. `update-subnumber`는 `--command` 인자를 지원하지 않으므로, command 변경이 필요한 경우 사용자에게 add-subnumber 분기(제출 후 편집)를 안내합니다.
