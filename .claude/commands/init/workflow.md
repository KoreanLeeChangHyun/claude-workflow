---
description: 워크플로우 초기화. CLAUDE.md 로드, workflow-agent-orchestration 스킬 로드. (매 세션마다)
---
# Initialize Workflow
...
## 2. workflow-agent-orchestration 스킬 로드

`.claude/skills/workflow-agent-orchestration/SKILL.md`를 로드합니다.
...
- `/init:workflow`는 **세션 초기화 전용**이며, 역할은 CLAUDE.md 로드 + workflow-agent-orchestration 스킬 로드뿐입니다
- 셸 스크립트 실행, 디렉터리 생성, 상태 파일 생성 등은 이 명령어의 범위가 아닙니다

> **절대 금지:** 이 명령어(`/init:workflow`) 실행 중 워크플로우 배너를 호출하지 않습니다.
...
CLAUDE.md 로드와 workflow-agent-orchestration 스킬 로드가 완료되면, Read 도구로 `.claude/scripts/data/help.txt`를 읽어 그 내용만 초기화 완료 메시지로 출력한다. 오케스트레이터는 이 파일의 내용 외에 어떠한 텍스트도 직접 출력하지 않는다.

## 4. 워크플로우 필수 준수 원칙

이후 모든 cc:* 명령어는 `workflow-agent-orchestration` 스킬의 워크플로우를 **반드시** 따릅니다.
...
상세는 `workflow-agent-orchestration` 스킬을 참조하세요.
