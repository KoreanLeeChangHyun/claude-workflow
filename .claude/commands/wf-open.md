...
| `-o` | 있음 | Step 1-B-o: 기존 티켓 열람 (내용 표시만, 편집 루프 미진입) |
...
## Step 1-B-o. `-o` 번호 있음: 기존 티켓 열람 (내용 표시만)

기존 티켓의 현재 내용을 표시합니다. 편집 루프에 진입하지 않습니다.
...
- 존재하면: Done 티켓임을 안내하고 내용을 표시합니다
- 어디에서도 찾지 못한 경우: 에러 출력 후 종료
  ```
  T-NNN 티켓 파일을 찾을 수 없습니다.
  ```

**상태 판별**: Read 도구로 로드한 XML에서 `<metadata>/<status>` 텍스트를 확인하여 아래 4가지 상태에 따라 분기합니다:

| 상태 | 분기 흐름 |
|------|----------|
| `Open` | 선택지 없이 바로 1-B-o-2로 진행 |
| `In Progress` | 1-B-o-1a 실행 (비-Open 상태 선택지 제시) |
| `Review` | 1-B-o-1a 실행 (비-Open 상태 선택지 제시) |
| `Done` | done 폴더 확인 후 내용 표시 (기존 동작 유지) |

### 1-B-o-1a. 비-Open 상태 선택지 제시

> **실행 조건**: 상태가 `In Progress` 또는 `Review`인 경우에만 실행합니다.

아래 선택지를 출력합니다:

```
`[T-NNN]` : `[WF -o]` T-NNN은 현재 **<상태>** 상태입니다.

`1.` 열람만 -- 내용을 표시합니다
`2.` Open으로 되돌리기 -- 상태를 Open으로 변경합니다
`0.` 취소 -- 작업을 종료합니다
```

**선택지 처리:**

- `1.` 열람만: 변경 없이 1-B-o-2로 진행합니다
- `2.` Open으로 되돌리기: Bash 도구로 아래 명령을 실행한 뒤 상태 변경 확인 메시지를 출력하고 1-B-o-2로 진행합니다
  ```bash
...
```
`[T-NNN]` : `[WF -o]` T-NNN 티켓 내용을 표시했습니다.
