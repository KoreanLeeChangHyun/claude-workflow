...
class WorkflowSessionRegistry:
    ...
    def create_external(
        ...
        """외부 process (v2 driver subprocess) 가 발급한 session_id 로 등록한다.

        Stage 3-B — board 측은 spawn 점이 아니므로 ClaudeProcess.spawn() 호출 안 함.
        process 인스턴스는 status='stopped' 유지하여 list_all() 출력 호환만 보존한다.
        SSE 채널 + persist 파일만 신설.

        동일 session_id 로 재호출 시 기존 세션을 그대로 반환한다 (idempotent).
        """
        with self._lock:
        ...
        if persist_path is not None:
            try:
                ...
                logger.error(
                    "workflow_session[%s]: external 메타 persist 실패 (%s): %s",
