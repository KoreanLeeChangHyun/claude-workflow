...
.claude-organic/workflow/ 및 .claude-organic/workflow/.history/ 디렉토리를 스캔하여 .claude-organic/dashboard/.history.md와 비교하고,
...
사용법:
    python3 .claude-organic/scripts/sync/history_sync.py sync [--workflow-dir <path>] [--target <path>] [--dry-run] [--all]
    python3 .claude-organic/scripts/sync/history_sync.py status [--workflow-dir <path>] [--target <path>] [--all]
    python3 .claude-organic/scripts/sync/history_sync.py archive [registryKey]
"""
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    .claude-organic/workflow/ 디렉터리를 스캔하여 history.md와 비교하고 누락/변경 항목을 동기화한다.
    ...
    # .claude-organic/workflow/ 스캔
    scanned = scan_workflow_directory(workflow_dir, include_all)
    if not scanned:
        print("[INFO] .claude-organic/workflow/ 디렉토리에 작업이 없습니다.")
        return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    .claude-organic/workflow/ 디렉터리와 history.md를 비교하여 동기화 상태 요약을 출력한다.
...
def _update_ticket_workdir_after_archive(moved_key: str, workflow_dir: str, history_dir: str) -> None:
    ...
    .claude-organic/workflow/.history/{key}/... 형태로 갱신한다.

    Args:
        moved_key: .history/로 이동된 워크플로우 키 (YYYYMMDD-HHMMSS 형식)
        workflow_dir: .claude-organic/workflow/ 디렉터리 절대 경로
        history_dir: .claude-organic/workflow/.history/ 디렉터리 절대 경로
    ...
    # 경로 갱신 함수: .claude-organic/workflow/{key}/... -> .claude-organic/workflow/.history/{key}/...
    def _rewrite_path(text: str) -> str:
        """workflow/{key}/를 workflow/.history/{key}/に置換."""
        if not text:
            return text
        old_prefix = f".claude-organic/workflow/{moved_key}/"
        new_prefix = f".claude-organic/workflow/.history/{moved_key}/"
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    KEEP_COUNT개를 초과하는 오래된 디렉터리를 .claude-organic/workflow/.history/로 이동한다.
    ...
    workflow_dir = os.path.join(PROJECT_ROOT, ".claude-organic", "workflow")
    history_dir = os.path.join(workflow_dir, ".history")
...
def main() -> int:
    ...
    sync_parser.add_argument("--workflow-dir", default=os.path.join(PROJECT_ROOT, ".claude-organic", "workflow"), help=".workflow 디렉토리 경로")
    sync_parser.add_argument("--target", default=os.path.join(PROJECT_ROOT, ".claude-organic", "dashboard", ".history.md"), help=".history.md 파일 경로")
    ...
    status_parser.add_argument("--workflow-dir", default=os.path.join(PROJECT_ROOT, ".claude-organic", "workflow"), help=".workflow 디렉토리 경로")
    status_parser.add_argument("--target", default=os.path.join(PROJECT_ROOT, ".claude-organic", "dashboard", ".history.md"), help=".history.md 파일 경로")
