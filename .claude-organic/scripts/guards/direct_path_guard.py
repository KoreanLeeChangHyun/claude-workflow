...
PreToolUse(Bash) 이벤트에서 python3 .claude-organic/scripts/ 패턴의 직접 경로 호출을
...
# 예(상대경로): python3 .claude-organic/hooks/... && python3 .claude-organic/scripts/sync/history_sync.py ...
# 예(절대경로): python3 /path/.claude-organic/hooks/... && python3 /path/.claude-organic/scripts/sync/history_sync.py ...
...
def _extract_script_name(command: str) -> str | None:
    """명령어에서 .claude-organic/scripts/ 하위 스크립트 파일명을 추출한다.
...
def main() -> None:
    ...
    stdin에서 JSON을 읽어 Bash 도구의 python3 .claude-organic/scripts/ 직접 호출을 감지하고,
    ...
    if script_name and script_name in ALIAS_MAP:
        ...
        # 스크립트명 추출 실패 시 일반 차단
        _deny(DIRECT_PATH_CALL_DENIED.format(
            script_name=".claude-organic/scripts/...",
            alias_name="flow-*",
        ))
