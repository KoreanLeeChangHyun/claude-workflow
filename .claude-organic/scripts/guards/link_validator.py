...
.claude-organic/workflow/ 디렉터리 내 report.md, plan.md 파일에서 마크다운 링크를 추출하고,
...
실행 예시:
    python3 .claude-organic/scripts/guards/link_validator.py
    python3 .claude-organic/scripts/guards/link_validator.py --active-only
...
# 프로젝트 루트 기준 경로 접두사
_PROJECT_ROOT_PREFIXES: tuple[str, ...] = (".claude-organic/", ".claude/")


def _find_project_root() -> Path:
    """프로젝트 루트 디렉터리를 찾아 반환한다.

    이 스크립트는 .claude-organic/scripts/guards/ 하위에 위치하므로,
...
def scan_markdown_files(
    ...
    .claude-organic/workflow/ 디렉터리 하위의 report.md, plan.md 파일을 수집한다.
    active_only=True이면 .claude-organic/workflow/.history/는 제외한다.

    Args:
        project_root: 프로젝트 루트 디렉터리 경로.
        active_only: True이면 활성 워크플로우(.claude-organic/workflow/ 직접 하위)만 스캔.
...
def validate_link(
    ...
    프로젝트 루트 기준 경로(.claude-organic/, .claude/ 시작)는 project_root에서 해석하고,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=".claude-organic/workflow/ 내 마크다운 파일의 링크 유효성을 검사합니다.",
    )
    parser.add_argument(
        "--active-only",
        action="store_true",
        default=False,
        help="활성 워크플로우(.claude-organic/workflow/ 직접 하위)만 검사합니다. .claude-organic/workflow/.history/는 제외.",
    )
