...
def _find_open_ticket_from_kanban() -> str | None:
    """.claude-organic/kanban/open/ 디렉터리의 XML 파일을 glob하여 Open 상태 첫 번째 티켓 번호를 반환한다.

    .claude-organic/kanban/open/*.xml 파일을 알파벳 순으로 탐색하며, <metadata> 내부의
    ...
    kanban_dir: str = os.path.join(_PROJECT_ROOT, ".claude-organic", "kanban")
...
def read_prompt(ticket_arg: str | None = None) -> tuple[str | None, str | None]:
    ...
    kanban_dir: Path = Path(_PROJECT_ROOT) / ".claude-organic" / "kanban"
...
def init_workflow(
    ...
    work_dir: str = f".claude-organic/workflow/{registry_key}/{work_name}/{command}"
    ...
    if os.path.exists(abs_work_dir):
        for suffix in range(1, 100):
            ...
            candidate_dir: str = f".claude-organic/workflow/{candidate_key}/{work_name}/{command}"
    ...
    workflow_root: str = os.path.join(_PROJECT_ROOT, ".claude-organic", "workflow")
