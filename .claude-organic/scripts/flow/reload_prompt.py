...
def _resolve_workdir_from_registry(registry_key: str) -> tuple[str | None, list[str]]:
    ...
    탐색 순서:
      1. .claude-organic/workflow/<registry_key>/ 하위 디렉터리 스캔
      2. 1차 탐색 실패(디렉터리 미존재) 시 .claude-organic/workflow/.history/<registry_key>/ 하위 스캔
    ...
    workflow_base = os.path.join(_PROJECT_ROOT, ".claude-organic", "workflow")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "예: .claude-organic/workflow/YYYYMMDD-HHMMSS/workName/command\n"
            "    20260403-011117"
        ),
    )
