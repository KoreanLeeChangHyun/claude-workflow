...
def _resolve_project_root() -> str:
    ...
    Returns:
        프로젝트 루트 절대 경로.
    """
    # flow_logger.py 위치: <project_root>/.claude-organic/scripts/flow/flow_logger.py
    # 서브에이전트 워크트리 위치:
    #   <main_root>/.claude/worktrees/agent-*/.claude-organic/scripts/flow/flow_logger.py
    this_file = os.path.abspath(__file__)
    flow_dir = os.path.dirname(this_file)          # .claude-organic/scripts/flow/
    scripts_dir = os.path.dirname(flow_dir)        # .claude-organic/scripts/
    claude_dir = os.path.dirname(scripts_dir)      # .claude-organic/
    candidate = os.path.dirname(claude_dir)        # <candidate>/

    # .claude-organic/.settings가 있으면 메인 리포 루트 — 즉시 반환
    cw_dir = os.path.join(candidate, ".claude-organic")
...
def _resolve_work_dir_from_key(
    ...
    """
    base_dir = os.path.join(project_root, ".claude-organic", "workflow", registry_key)
