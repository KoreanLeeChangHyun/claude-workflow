...
def _update_usage_md(row: str, eff_weighted: float) -> str | None:
    """.claude-organic/dashboard/.usage.md 파일에 사용량 행을 삽입한다.
    ...
    """
    usage_md = os.path.join(PROJECT_ROOT, ".claude-organic", "dashboard", ".usage.md")
...
def usage_regenerate() -> str:
    """.claude-organic/workflow/ 및 .claude-organic/workflow/.history/ 하위의 모든 usage.json을 순회하여 .claude-organic/dashboard/.usage.md를 재생성한다.
    ...
    try:
        ...
        workflow_base = os.path.join(PROJECT_ROOT, ".claude-organic", "workflow")
