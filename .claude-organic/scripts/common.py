...
주요 함수:
    ...
    read_env: .claude-organic/.settings 환경변수 읽기
"""
...
def _detect_worktree_main_root(base: str) -> str:
    ...
    """
    # .claude-organic/.settings가 있으면 이미 메인 리포 루트 — git 호출 불필요
    cw_dir = os.path.join(base, ".claude-organic")
...
def scan_active_workflows(
    ...
    """.claude-organic/workflow/ 디렉터리를 스캔하여 워크플로우 목록을 반환.

    .claude-organic/workflow/<YYYYMMDD-HHMMSS>/<workName>/<command>/ 구조를 순회하며
    ...
    workflow_root = os.path.join(project_root, ".claude-organic", "workflow")
    ...
    for entry in os.listdir(workflow_root):
        ...
        # 중첩 구조: .claude-organic/workflow/<YYYYMMDD-HHMMSS>/<workName>/<command>/
        # 동일 entry에 복수 워크플로우가 있을 경우 updated_at 최신순으로 결정적 선택
        ...
        for work_name in sorted(os.listdir(entry_path)):
            ...
            for cmd_name in sorted(os.listdir(wn_path)):
                ...
                rel_work_dir = os.path.join(".claude-organic", "workflow", entry, work_name, cmd_name)
                candidates.append({
                    "title": title,
                    "step": phase,
...
def resolve_work_dir(input_key: str, project_root: str | None = None) -> str:
    ...
    탐색 순서:
      1. .claude-organic/workflow/<input_key>/ 하위 디렉터리 스캔
      2. 1차 탐색 실패 시 .claude-organic/workflow/.history/<input_key>/ 하위 디렉터리 스캔
      3. 두 탐색 모두 실패 시 ".claude-organic/workflow/<input_key>" 폴백 반환
    ...
    Returns:
        해석된 workDir 상대 경로. 스캔 실패 시 ".claude-organic/workflow/<input_key>" 폴백.
    """
    ...
    base_dir = os.path.join(project_root, ".claude-organic", "workflow", input_key)
    result = _scan_base(base_dir, os.path.join(".claude-organic", "workflow", input_key))
    if result is not None:
        return result

    # 2차 탐색: .history/ 아카이브 디렉터리
    history_base_dir = os.path.join(project_root, ".claude-organic", "workflow", ".history", input_key)
    result = _scan_base(history_base_dir, os.path.join(".claude-organic", "workflow", ".history", input_key))
    if result is not None:
        return result

    # 폴백
    fallback = f".claude-organic/workflow/{input_key}"
    print(
...
# =============================================================================
# 환경변수 파싱 (.claude-organic/.settings)
# =============================================================================
...
def _resolve_env_file(env_file: str | None = None) -> str:
    ...
    return os.path.join(project_root, ".claude-organic", ".settings")


def read_env(key: str, default: str = "", env_file: str | None = None) -> str:
    """.claude-organic/.settings에서 환경변수 읽기.
