...
def resolve_work_dir(input_key: str, project_root: str | None = None) -> str:
    ...
    Returns:
        해석된 workDir 상대 경로. 스캔 실패 시 ".claude-organic/runs/<input_key>" 폴백.
    """
    ...
    base_dir = os.path.join(project_root, ".claude-organic", "runs", input_key)
    result = _scan_base(base_dir, os.path.join(".claude-organic", "runs", input_key))
    if result is not None:
        return result

    # 2차 탐색: .history/ 아카이브 디렉터리
    history_base_dir = os.path.join(project_root, ".claude-organic", "runs", ".history", input_key)
    result = _scan_base(history_base_dir, os.path.join(".claude-organic", "runs", ".history", input_key))
    if result is not None:
        return result

    # 폴백
    fallback = f".claude-organic/runs/{input_key}"
    print(
