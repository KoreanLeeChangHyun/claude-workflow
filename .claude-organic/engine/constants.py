...
.claude-organic/engine/ 하위 스크립트에서 공통 사용하는 상수, 패턴, 매핑을 한 곳에 정의합니다.
...
def _load_dotenv() -> dict[str, str]:
    """.claude-organic/.settings 파일을 파싱하여 key=value dict로 반환한다.
    ...
    """
    cw_dir = os.path.join(_find_project_root(), '.claude-organic')
...
DANGER_PATTERNS = [
    ...
    {"pattern": "(sudo\\s+)?rm\\s+-r[f]*\\s+.*\\.claude\\.workflow/kanban", "blocked": "rm -rf .claude-organic/kanban (칸반 디렉터리 삭제)", "alternative": "칸반 디렉터리는 워크플로우 핵심 데이터입니다. 삭제하지 마세요."},
]
...
GUARD_PROTECTED_PATH_PATTERNS = [
    "\\.claude-organic/hooks/",
    "\\.claude-organic/runs/bypass",
]
