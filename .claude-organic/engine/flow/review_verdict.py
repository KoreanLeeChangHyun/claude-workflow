...
"""review_verdict.py - Review 단계 1차 룰베이스 자동 검증 (T-463 W21).
...
캐논:
    ...
    - T-411 commit 0c970fa (status 강제 전이 폐기)
    - T-413 commit 1ce3c2d (Auditor 사이드카 폐지)

based on W11 §A: finalization.py L861 hook insertion (W04 회귀 패턴 캡처
직후, Step 4 kanban move review 직전 위치). based on W11 §C: T-454
phase_verifier.py VerifyResult 패턴(_find_git_root / _check_commits_ahead).

13 룰 카탈로그 (workflow.md §3 와 동기 박제 - W23 단일 진실 공급원):
