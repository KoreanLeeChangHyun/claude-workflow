...
from constants import KST, KEEP_COUNT, VALID_COMMANDS, VALID_MODES, WORK_NAME_MAX_LEN, parse_chain_command, CHAIN_SEPARATOR
...
def _find_open_ticket_from_kanban() -> str | None:
    """.claude-organic/tickets/open/ 디렉터리의 XML 파일을 glob하여 Open 상태 첫 번째 티켓 번호를 반환한다.

    .claude-organic/tickets/open/*.xml 파일을 알파벳 순으로 탐색하며, <metadata> 내부의
...
def init_workflow(
    ...
    workflow_root: str = os.path.join(_PROJECT_ROOT, ".claude-organic", "runs")
