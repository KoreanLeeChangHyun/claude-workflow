...
def _resolve_project_root() -> str:
    ...
    Returns:
        프로젝트 루트 절대 경로.
    """
    # flow_logger.py 위치: <project_root>/.claude-organic/engine/flow/flow_logger.py
    # 서브에이전트 워크트리 위치:
    #   <main_root>/.claude/worktrees/agent-*/.claude-organic/engine/flow/flow_logger.py
    this_file = os.path.abspath(__file__)
    flow_dir = os.path.dirname(this_file)          # .claude-organic/engine/flow/
    scripts_dir = os.path.dirname(flow_dir)        # .claude-organic/engine/
...
def _resolve_work_dir_from_key(
    ...
    """
    base_dir = os.path.join(project_root, ".claude-organic", "runs", registry_key)
