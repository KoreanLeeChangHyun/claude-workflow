...
사용법:
    python3 .claude-organic/engine/sync/catalog_sync.py              # 카탈로그 생성/갱신
    python3 .claude-organic/engine/sync/catalog_sync.py --dry-run     # 미리보기 (파일 쓰기 없음)
...
def main() -> None:
    ...
    if dry_run:
        ...
        print()
        print("  실제 생성하려면: python3 .claude-organic/engine/sync/catalog_sync.py")
        print(flush=True)
        sys.exit(0)
