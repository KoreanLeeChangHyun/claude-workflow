...
| VALIDATE | driver → claude -p | 1 spawn | `validate-report.md` | 12룰 advisory 평가, 추가 quality 검증 |
...
| VALIDATE 12룰 평가 (claude -p advisory) | claude -p (advisory) — validate-report.md 산출 |
| driver 룰베이스 12룰 재검증 (DONE 시점) | DONE Step 안에서 `evaluate_12_rules` + `save_verdict_report` (validate-rules.json) — VALIDATE 시점은 report.md / step.end DONE 미생성으로 거짓 FAIL, 따라서 REPORT 완료 + step.end DONE 기록 후가 정합 시점 |
...
def main(ticket_no: str) -> int:
    ...
    verdict = evaluate_12_rules(ctx)  # 룰베이스 재검증 (advisory)
...
| R-WT | R-WT-1 | commits ahead ≥ 1 또는 SKIP | YES (T-486 Phase 2b 에서 승격) |
