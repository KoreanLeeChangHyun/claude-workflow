...
.claude-organic/runs/ 디렉터리 내 report.md, plan.md 파일에서 마크다운 링크를 추출하고,
...
실행 예시:
    python3 .claude-organic/engine/guards/link_validator.py
    python3 .claude-organic/engine/guards/link_validator.py --active-only
...
def _find_project_root() -> Path:
    ...
    이 스크립트는 .claude-organic/engine/guards/ 하위에 위치하므로,
...
def scan_markdown_files(
    ...
    .claude-organic/runs/ 디렉터리 하위의 report.md, plan.md 파일을 수집한다.
    active_only=True이면 .claude-organic/runs/.history/는 제외한다.

    Args:
        project_root: 프로젝트 루트 디렉터리 경로.
        active_only: True이면 활성 워크플로우(.claude-organic/runs/ 직접 하위)만 스캔.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=".claude-organic/runs/ 내 마크다운 파일의 링크 유효성을 검사합니다.",
    )
    parser.add_argument(
        "--active-only",
        action="store_true",
        default=False,
        help="활성 워크플로우(.claude-organic/runs/ 직접 하위)만 검사합니다. .claude-organic/runs/.history/는 제외.",
    )
