#!/bin/bash
set -euo pipefail
# init-claude-workflow.sh — Claude Code 워크플로우 환경 자동 초기화 스크립트
# 지원: Ubuntu 20.04+, macOS 13.0+ | 의존성: git, curl, python3, tmux, gh

# --- 상수 로드 ---
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULTS_CONF="${SCRIPT_DIR}/.claude/init/defaults.conf"
if [ ! -f "$DEFAULTS_CONF" ]; then
    echo "ERROR: defaults.conf not found: $DEFAULTS_CONF" >&2
    exit 1
fi
# shellcheck source=.claude/init/defaults.conf
...
validate_templates() {
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        ...
        print_info "init-claude-workflow.sh 실행 전 .claude/init/templates/ 디렉터리를 확인하세요."
        exit 1
    fi
}
...
generate_claude_env() {
    ...
    [ -f "$env_file" ] && { print_info ".claude.env 파일이 이미 존재합니다. 기존 내용을 유지합니다."; return 0; }
...
migrate_legacy_prompt() {
    ...
    else print_success "${copied}개 파일을 .kanban/으로 복사 완료"; fi
...
verify_installation() {
    ...
    [ "$kanban_ok" = true ] && print_success ".kanban/, .kanban/done/ 디렉터리 존재 확인"
    # (e) .claude.env 파일 존재
    if [ -f ".claude.env" ]; then print_success ".claude.env 파일 존재 확인"
    else print_error ".claude.env 파일이 없습니다"; failed=$((failed + 1)); fi
    # (f) .claude/ 디렉터리 존재
